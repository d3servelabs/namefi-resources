...
export async function generateMetadata({
  ...
  const entry = getGlossaryCached(locale, slug);
...
export default async function GlossaryDetailPage({
  ...
  const keywordsHeading =
    dictionary.glossary.detailKeywordsHeading ??
  ...
  return (
    <article className="mx-auto flex w-full max-w-6xl flex-col gap-10 px-6 py-12 text-start md:px-10 lg:px-12">
      ...
      >
        {dictionary.glossary.detailBack}
      </Link>
      ...
      {keywords.length > 0 && (
        <section className="space-y-4">
          ...
          <ul className="flex flex-wrap gap-3 text-xs uppercase tracking-wide text-muted-foreground">
