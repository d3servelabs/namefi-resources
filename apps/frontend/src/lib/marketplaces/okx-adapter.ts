...
/**
 * Legacy OKX NFT marketplace adapter — superseded by the Seaport-based
 * `OkxAdapter` (`okx-adapter.ts`); kept as a reference / fallback, not wired
 * into `getMarketplace()`.
 *
 ...
 *   - createListing → proxy `okx.createListing` returns approval + EIP-712
 *     steps; the adapter runs the NFT approval, signs the Seaport order, and
 *     relays the signature via proxy `okx.submitListing`
...
export class OkxAdapter implements MarketPlace {
  ...
  async calculateListingFees(input: {
    ...
    const totalBps = OKX_PROTOCOL_FEE_BPS + royaltyFeeBps;
    const feeWei = (input.priceWei * BigInt(totalBps)) / 10_000n;
    return {
      marketplaceFeeBps: OKX_PROTOCOL_FEE_BPS,
      royaltyFeeBps,
  ...
  async createListing(input: ListingInput): Promise<Listing> {
    ...
    try {
      response = OkxCreateListingResponseSchema.parse(
        await marketplaceProxyClient.nftMarketplaces.okx.createListing.mutate({
          chain: this.okxChainKey,
          walletAddress: wallet.address,
          items: [
            {
              collectionAddress: input.tokenAddress,
              tokenId: input.tokenId,
              price: input.priceWei.toString(),
              currencyAddress: currency.contract,
              count: 1,
              validTime,
              platform: OKX_LISTING_PLATFORM,
            },
          ],
        }),
      );
    } catch (error) {
      // OKX has disabled its create-listing endpoint (it returns
      // `{ code: -1, "No longer available" }`) — surface that as a clean
      // unsupported-operation error rather than a raw 500.
