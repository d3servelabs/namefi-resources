...
export const Landing: LandingComponent = ({ origin }) => {
  ...
  const { data: rolloutPercent, isLoading: isLoadingRolloutPercent } = useQuery(
    trpc.registry.get0xDotCityPercentageRollout.queryOptions(), //todo
  );
  ...
  const parentDomain = origin.thirdPartyHostname
    .replace('.localhost', '')
    .replace('astra.namefi.io', '')
    .replace('poweredby.namefi.io', '');
  // Capitalize the first letter of the first part for proper sentence case
  const prettyParentDomain =
    parentDomain.charAt(0).toUpperCase() + parentDomain.slice(1);
    ...
    <div className="relative flex gap-4 flex-col p-4 pb-0 pt-20">
      <div className="flex flex-col items-center gap-4">
        <SearchHeader
          ...
          tagline={`${prettyParentDomain}: Find your next domain`}
        />
        <SearchInput
          query={query}
          setQuery={setQuery}
          ...
          {isNewsletterVisible && (
            ...
            >
              <div className="w-full max-w-screen-xl">
                <NewsletterForm
                  from={origin.thirdPartyHostname ?? 'namefi'}
                  title="Stay in the Loop"
                  description={`Get the latest updates on ${origin.thirdPartyHostname ?? 'namefi'} domain releases, features, and announcements.`}
                  showNameField={true}
                  variant="default"
...
Landing.displayName = 'FallbackThirdPartyLanding';
