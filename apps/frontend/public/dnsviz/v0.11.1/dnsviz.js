...
AuthGraph.prototype.infoToHtmlTable = function (obj) {
  return (
    '<table class="obj">' + this.infoToHtmlTableComponents(obj) + '</table>'
  );
};
...
AuthGraph.prototype.infoToHtmlTableComponents = function (obj) {
  ...
  for (var key in obj) {
    ...
    if (
      key.toLowerCase() in
      { digest: null, key: null, signature: null, dnskey: null }
    ) {
      // don't print digest or key
      continue;
    }
    if (
      key.toLowerCase() in { rdata: null, meta: null } &&
      !Object.hasOwn(val, 'length')
    ) {
    ...
    s +=
      ...
      this.labelFromSlug(key) +
      ':</th><td align="left">';
    if (typeof val != 'object') {
      s += val;
    } else if (Object.hasOwn(val, 'length')) {
      if (
        key.toLowerCase() in { nsec: null, nsec3: null } &&
        !Object.hasOwn(val, 'object')
      ) {
        var newval = [];
        var nsecType = key.toLowerCase() == 'nsec' ? 'NSEC' : 'NSEC3';
        for (var i = 0; i < val.length; i++) {
          newval.push(
            val[i]['name'] + ' IN ' + nsecType + ' ' + val[i]['rdata'][0],
          );
        }
        val = newval;
      }
      if (key.toLowerCase() in { errors: null, warnings: null }) {
        ...
        for (var i = 0; i < val.length; i++) {
          ...
          if (val[i]['servers'] != undefined) {
            serversTags = serversTags.concat(val[i]['servers']);
          }
          if (val[i]['query_options'] != undefined) {
            serversTags = serversTags.concat(val[i]['query_options']);
          }
          if (serversTags.length > 0) {
            s += ' (' + serversTags.join(', ') + ')';
          }
          s += '</li>';
        }
...
AuthGraph.prototype.addNodeEvent = function (nodeObj, infoObj) {
  var statusStr;
  var s = '';
  if (Object.hasOwn(infoObj, 'length') && infoObj.length > 1) {
    ...
    if (Object.hasOwn(infoObj, 'length')) {
      infoObj = infoObj[0];
    }
...
AuthGraph.prototype.slugify = (str) => {
