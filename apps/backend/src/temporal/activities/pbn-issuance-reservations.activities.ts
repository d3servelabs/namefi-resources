...
import { InternalReservationNotification } from '../../mail/templates/internal-reservation-notification';
...
export async function createReservation(
  ...
  // Check for existing active free-claim reservation for the same recipient+exact
  const existingReservation =
    issueFreeClaim && exactDomainName
      ? await db
          .select()
          ...
          .where(
            and(
              eq(
                pbnIssuanceReservationsTable.recipientEmail,
                normalizedRecipientEmail as string,
              ),
  ...
  if (sendEmail && normalizedRecipientEmail) {
    try {
      const emailTemplate = issueFreeClaim
        ? React.createElement(GiftReservationNotification, {
            ...
            ...(reservationExpirationDate
              ? { expirationDate: reservationExpirationDate.toISOString() }
              : {}),
            poweredByNamefiDomain: pbnDomain,
          })
        : React.createElement(InternalReservationNotification, {
            recipientEmail: normalizedRecipientEmail,
            pbnDomain: pbnDomain as NamefiNormalizedDomain,
            exactDomainName: exactDomainName as NamefiNormalizedDomain,
            parentDomain: parentDomain as NamefiNormalizedDomain,
            ...
            ...(reservationExpirationDate
              ? {
                  expirationDate:
                    reservationExpirationDate.toISOString() as string,
                }
              : {}),
            poweredByNamefiDomain: pbnDomain,
          });
