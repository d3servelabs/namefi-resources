...
vi.mock('bottleneck', () => {
  class IoRedisConnectionMock {
    on = vi.fn();
  }

  class BottleneckMock {
    connection = { on: vi.fn() };
    on = vi.fn();
  ...
  Object.defineProperty(BottleneckMock, 'IORedisConnection', {
    value: IoRedisConnectionMock,
  });

  return { default: BottleneckMock };
});
...
describe('generateLogoAnimation', () => {
  ...
  it('fails the claimed animation when the reference logo is missing before pickup', async () => {
    ...
    const whereGeneration = vi
      ...
      .mockResolvedValueOnce([{ metadata: generation.metadata }]);
  ...
  it('keeps looped animations submitted until the gateway video slot succeeds', async () => {
    vi.useFakeTimers();
    ...
    const generation = {
      ...
      finishedAt: null,
      updatedAt: new Date('2026-03-25T00:00:00.000Z'),
      referenceGenerationId: 'logo-queued',
      domain: 'brand.xyz',
      metadata: {},
      input: {
        type: 'animation',
      ...
      output: {
        type: 'animation',
        thumbnailStoragePath: 'logos/logo-queued.png',
        mimeType: 'video/mp4',
        model: 'bytedance/seedance-v1.5-pro',
      },
    };
    ...
    const referenceLogo = {
      ...
      output: {
        type: 'logo',
        storagePath: 'logos/logo-queued.png',
      },
    };
    const quotaMetadata = {
    ...
    const completedQuotaMetadata = {
      vercelGatewayVideoQuota: {
        ...quotaMetadata.vercelGatewayVideoQuota,
        state: 'succeeded',
        completedAt: '2026-03-25T00:00:10.000Z',
      },
    };
    const waitingQuotaMetadata = {
      vercelGatewayVideoQuota: {
        ...quotaMetadata.vercelGatewayVideoQuota,
        state: 'waiting',
        lastRateLimitedAt: '2026-03-25T00:00:02.000Z',
      },
    };
    ...
    const selectWhereMock = vi
      ...
      .mockResolvedValueOnce([referenceLogo])
      .mockResolvedValueOnce([{ metadata: generation.metadata }])
      .mockResolvedValueOnce([{ metadata: quotaMetadata }])
      .mockResolvedValueOnce([{ metadata: waitingQuotaMetadata }])
      .mockResolvedValueOnce([{ metadata: quotaMetadata }])
      .mockResolvedValueOnce([{ metadata: completedQuotaMetadata }])
      .mockResolvedValueOnce([{ metadata: completedQuotaMetadata }]);
    const selectFromMock = vi.fn(() => ({ where: selectWhereMock }));
    dbMock.select.mockImplementation(() => ({ from: selectFromMock }));
    ...
    const quotaError = new Error(
      'Video generation has a quota of 1 request per minute for balances below $100.',
    );
    const providerOperation = vi
      .fn()
      .mockRejectedValueOnce(quotaError)
      .mockResolvedValueOnce({ ok: true });
    runLogoAnimationWorkflowMock.mockImplementation(
      async (
        _input: unknown,
        options: {
          runVercelGatewayVideoGeneration: (
            context: {
              mode: 'looped';
              model: 'bytedance/seedance-v1.5-pro';
              provider: 'vercel-ai-gateway';
        ...
        await options.runVercelGatewayVideoGeneration(
          {
            mode: 'looped',
            model: 'bytedance/seedance-v1.5-pro',
            provider: 'vercel-ai-gateway',
    ...
    const promise = generateLogoAnimation({ generationId: generation.id });
    ...
    expect(providerOperation).toHaveBeenCalledTimes(2);
    type GatewayProcessingUpdate = {
      metadata?: {
        vercelGatewayVideoQuota?: {
          state?: string;
    ...
    const gatewayProcessingUpdates = (
      updateSetMock.mock.calls as unknown as Array<[GatewayProcessingUpdate]>
    )
      .map(([payload]) => payload)
      .filter(
        (
          payload,
        ): payload is GatewayProcessingUpdate & {
          metadata: {
            vercelGatewayVideoQuota: {
              state: string;
              errorMessage?: string | null;
        ...
        } => payload.metadata?.vercelGatewayVideoQuota?.state === 'processing',
      );
    expect(gatewayProcessingUpdates).toHaveLength(2);
    for (const payload of gatewayProcessingUpdates) {
      expect(payload).not.toHaveProperty('startedAt');
      expect(payload.metadata.vercelGatewayVideoQuota).toEqual(
        expect.objectContaining({
          errorMessage: null,
          nextRetryAt: null,
        }),
    ...
    expect(updateSetMock).toHaveBeenCalledWith(
      expect.objectContaining({
        metadata: expect.objectContaining({
          vercelGatewayVideoQuota: expect.objectContaining({
            provider: 'vercel-ai-gateway',
            limiter: 'bottleneck',
            state: 'processing',
            mode: 'looped',
  ...
  it('stops gateway video retries when cancelled during quota backoff', async () => {
    vi.useFakeTimers();

    const cancellationController = new AbortController();
    temporalContextMock.current.mockReturnValueOnce({
      cancellationSignal: cancellationController.signal,
      heartbeat: vi.fn(),
    });

    const generation = {
      id: 'animation-cancelled-backoff',
      type: 'animation',
      status: 'PENDING',
      isDeleted: false,
      startedAt: null,
      finishedAt: null,
      updatedAt: new Date('2026-03-25T00:00:00.000Z'),
      referenceGenerationId: 'logo-cancelled-backoff',
      domain: 'brand.xyz',
      metadata: {},
      input: {
        type: 'animation',
      ...
      output: {
        type: 'animation',
        thumbnailStoragePath: 'logos/logo-cancelled-backoff.png',
        mimeType: 'video/mp4',
        model: 'bytedance/seedance-v1.5-pro',
      },
    };
    ...
    const referenceLogo = {
      id: 'logo-cancelled-backoff',
      type: 'logo',
      isDeleted: false,
      output: {
        type: 'logo',
        storagePath: 'logos/logo-cancelled-backoff.png',
      },
    };
    const quotaMetadata = {
      vercelGatewayVideoQuota: {
  ...
  it('skips duplicate gateway video attempts when the pending claim is already gone', async () => {
    const generation = {
      id: 'animation-duplicate',
      type: 'animation',
      status: 'PENDING',
      isDeleted: false,
      startedAt: null,
      finishedAt: null,
      updatedAt: new Date('2026-03-25T00:00:00.000Z'),
      referenceGenerationId: 'logo-duplicate',
      domain: 'brand.xyz',
      metadata: {},
      input: {
        type: 'animation',
      ...
      output: {
        type: 'animation',
        thumbnailStoragePath: 'logos/logo-duplicate.png',
        mimeType: 'video/mp4',
        model: 'bytedance/seedance-v1.5-pro',
      },
    };
    ...
    const selectWhereMock = vi
      .fn()
      .mockResolvedValueOnce([generation])
      .mockResolvedValueOnce([{ status: 'SUCCEEDED', isDeleted: false }]);
    const selectFromMock = vi.fn(() => ({ where: selectWhereMock }));
    dbMock.select.mockImplementation(() => ({ from: selectFromMock }));
    ...
    const providerOperation = vi.fn().mockResolvedValue({ ok: true });
    runLogoAnimationWorkflowMock.mockImplementation(
      async (
        _input: unknown,
        options: {
  ...
  it('fails immediately on non-quota Vercel Gateway video errors', async () => {
    const generation = {
      id: 'animation-provider-failure',
      type: 'animation',
      status: 'PENDING',
      isDeleted: false,
      startedAt: null,
      finishedAt: null,
      updatedAt: new Date('2026-03-25T00:00:00.000Z'),
      referenceGenerationId: 'logo-provider-failure',
      domain: 'brand.xyz',
      metadata: {},
      input: {
        type: 'animation',
      ...
      output: {
        type: 'animation',
        thumbnailStoragePath: 'logos/logo-provider-failure.png',
        mimeType: 'video/mp4',
        model: 'bytedance/seedance-v1.5-pro',
      },
    };
    ...
    const referenceLogo = {
      id: 'logo-provider-failure',
      type: 'logo',
      isDeleted: false,
      output: {
        type: 'logo',
        storagePath: 'logos/logo-provider-failure.png',
      },
    };
    ...
    const selectWhereMock = vi
      ...
      .mockResolvedValueOnce([{ metadata: generation.metadata }])
      .mockResolvedValueOnce([{ metadata: quotaMetadata }])
      .mockResolvedValueOnce([{ metadata: failedQuotaMetadata }]);
    const selectFromMock = vi.fn(() => ({ where: selectWhereMock }));
    dbMock.select.mockImplementation(() => ({ from: selectFromMock }));
    ...
    const providerError = new Error('provider failed before job creation');
    const providerOperation = vi.fn().mockRejectedValue(providerError);
    runLogoAnimationWorkflowMock.mockImplementation(
      async (
        _input: unknown,
        options: {
    ...
    await expect(
      generateLogoAnimation({ generationId: generation.id }),
    ).rejects.toThrow('provider failed before job creation');
  ...
  it('fails looped animations when the gateway quota retry limit is exhausted', async () => {
    const generation = {
      id: 'animation-quota-exhausted',
      type: 'animation',
      status: 'PENDING',
      isDeleted: false,
      startedAt: null,
      finishedAt: null,
      updatedAt: new Date('2026-03-25T00:00:00.000Z'),
      referenceGenerationId: 'logo-quota-exhausted',
      domain: 'brand.xyz',
      metadata: {},
      input: {
        type: 'animation',
      ...
      output: {
        type: 'animation',
        thumbnailStoragePath: 'logos/logo-quota-exhausted.png',
        mimeType: 'video/mp4',
        model: 'bytedance/seedance-v1.5-pro',
      },
    };
    ...
    const referenceLogo = {
      id: 'logo-quota-exhausted',
      type: 'logo',
      isDeleted: false,
      output: {
        type: 'logo',
        storagePath: 'logos/logo-quota-exhausted.png',
      },
    };

    const selectWhereMock = vi.fn().mockResolvedValue([{ metadata: {} }]);
    selectWhereMock
      .mockResolvedValueOnce([generation])
      .mockResolvedValueOnce([referenceLogo]);
    const selectFromMock = vi.fn(() => ({ where: selectWhereMock }));
    ...
    const returningMock = vi
      .fn()
      .mockResolvedValue([{ ...generation, status: 'PROCESSING' }]);
    const updateWhereMock = vi.fn(() => ({ returning: returningMock }));
    const updateSetMock = vi.fn(() => ({ where: updateWhereMock }));
    dbMock.update.mockImplementation(() => ({ set: updateSetMock }));
    ...
    const providerOperation = vi.fn().mockRejectedValue(quotaError);
    runLogoAnimationWorkflowMock.mockImplementation(
      async (
        _input: unknown,
        options: {
    ...
    try {
      await expect(
        ...
        'Vercel AI Gateway video quota remained unavailable after 15 retries',
      );
    } finally {
      setTimeoutSpy.mockRestore();
    }

    expect(providerOperation).toHaveBeenCalledTimes(16);
    expect(updateSetMock).toHaveBeenCalledWith(
      expect.objectContaining({
        metadata: expect.objectContaining({
          vercelGatewayVideoQuota: expect.objectContaining({
