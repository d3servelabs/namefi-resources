...
export const wishlistRouter = createContractTRPCRouter<typeof wishlistContract>(
  {
    addToWishlist: protectedProcedure
      .input(wishlistContract.addToWishlist.input)
      .output(wishlistContract.addToWishlist.output)
    ...
    removeFromWishlist: protectedProcedure
      .input(wishlistContract.removeFromWishlist.input)
      .output(wishlistContract.removeFromWishlist.output)
      .mutation(async ({ input, ctx }) => {
        const { user } = ctx;
    ...
    getWishlistDomains: protectedProcedure
      .input(wishlistContract.getWishlistDomains.input)
      .output(wishlistContract.getWishlistDomains.output)
      .query(async ({ ctx }) => {
        const { user, poweredByNamefiDomain } = ctx;
        const wishlistedDomains =
          await db.query.wishlistedDomainsTable.findMany({
            where: and(
              eq(wishlistedDomainsTable.userId, user.id),
