...
app.get('/test-failure', (c) => {
  const errorType = (z.coerce
    .number()
    .optional()
    .default(500)
    .safeParse(c.req.query('errorType')).data ?? 500) as ContentfulStatusCode;

  throw new HTTPException(errorType, { message: `Test Error ${errorType}` });
});
...
app.onError((err, c) => {
  ...
  const routeInfo = {
    source: 'hono' as const,
    method: c.req.method,
    path: c.req.path,
    url: c.req.url,
    requestId: c.get('requestId'),
  };
  const ignoreAlert = ignorePaths.some((ignorePath) =>
    c.req.path.startsWith(ignorePath),
  );
  if (err instanceof HTTPException) {
    // Alerts only fire for 400 / 5XX (handled inside sendHttpAlert).
    if (!ignoreAlert)
      void sendHttpAlert(err.status, err, err.message, routeInfo);
  ...
  logger.error(err);
  if (!ignoreAlert) void sendHttpAlert(500, err, err.message, routeInfo);
