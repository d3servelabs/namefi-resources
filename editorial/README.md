...
| `topic-slate-2026-09.html` | One pass of pipeline stations S1–S3, as of 2026-09-01: 100 topic candidates spread across the catalog's six subjects and four stages, ranked on measured demand proxies (Wikimedia Pageviews, HN Algolia, the repo's own `glossary:mentions`) because no search-volume API is provisioned here; ten source-scouted pitch memos with fact-list counts, source ledgers and PASS/THIN/FAIL verdicts; ten multi-persona debate summaries; a Decisions section for gate G1; and a Method section separating measured numbers from judgment calls. Bilingual, sortable, opens from `file://`. |
