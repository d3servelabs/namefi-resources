...
- Frontend GA recommended ecommerce events such as `add_to_cart` and `begin_checkout` are browser-side interaction events. The standard GA4 ecommerce `purchase` event is emitted from the backend order workflow only after at least one domain or NFSC top-up item succeeds. Send one `purchase` per order and include only succeeded items; failed or cancelled items belong in custom checkout diagnostics such as `domain_acquisition_finished` or `order_item_processing_finished`. NFSC top-up order placement uses `order_source: nfsc_topup` and then follows the same backend lifecycle event sequence as domain orders. Actual refund completions emit the custom `payment_refunded` funnel event, not GA4's standard ecommerce `refund`, because failed items are not included in the ecommerce `purchase`. The admin checkout flow dashboard intentionally reads the backend checkout event sequence (`user_begin_search`, `order_placed`, `payment_processed`, etc.) instead, and `order_placed` means the order was accepted for processing, not purchased.
