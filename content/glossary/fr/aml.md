---
title: AML
date: '2026-06-22'
language: fr
tags: ['glossary']
authors: ['namefiteam']
editors: ['victor-zhou']
translators: ['alan-machin']
description: Anti-Money-Laundering — surveillance continue des transactions et déclarations utilisées par les services réglementés pour détecter les fonds illicites.
keywords: ['AML', 'Anti-Money-Laundering', 'surveillance des transactions', 'filtrage des sanctions', 'conformité']
also_known_as: ['Anti-Money-Laundering']
level: 1
sources:
  - https://www.fincen.gov/resources/statutes-regulations/guidance
  - https://www.investopedia.com/terms/a/aml.asp
relatedArticles:
  - /fr/blog/from-mona-co-to-crypto-com/
  - /fr/blog/what-are-stablecoins/
  - /fr/blog/the-2024-squarespace-defi-domain-hijacks/
  - /fr/blog/choosing-a-domain-tokenization-platform/
  - /fr/blog/the-manifesto-of-digital-trust/
relatedTopics:
  - /fr/topics/web3-foundations/
  - /fr/topics/domain-investing/
relatedSeries:
  - /fr/series/domain-apocalypse/
  - /fr/series/name-change-game-change/
relatedGlossary:
  - /fr/glossary/kyc/
  - /fr/glossary/stablecoin/
  - /fr/glossary/icann/
  - /fr/glossary/smart-contract/
  - /fr/glossary/web3/
---

L'**AML** (*Anti-Money-Laundering*) est la surveillance continue des transactions et la déclaration que les services financiers et crypto réglementés utilisent pour détecter et dissuader les fonds illicites — en contrôlant les activités par rapport aux listes de sanctions et en signalant les schémas inhabituels aux autorités telles que le FinCEN aux États-Unis. Là où le [KYC](/fr/glossary/kyc/) vérifie l'identité d'un utilisateur lors de l'inscription, l'AML surveille ce qu'il fait ensuite. Dans le domaine des cryptomonnaies, les émetteurs de [stablecoins](/fr/glossary/stablecoin/) réglementés et les services de garde peuvent geler ou mettre sur liste noire les comptes en [conservation tierce](/fr/glossary/custodial-ownership/) qui échouent à ces contrôles de conformité, tandis que les protocoles non gardiens n'ont généralement aucune partie en mesure de les faire respecter.
