---
title: 协议资产
date: '2025-06-30'
language: zh-CN
tags: ["glossary"]
authors: ["namefiteam"]
description: 什么是协议资产，代币化域名如何作为协议资产发挥作用？
keywords: ["协议资产","区块链协议","基础设施资产","网络资产","实用代币"]
relatedArticles:
  - /zh-CN/blog/what-are-tokenized-domains/
  - /zh-CN/blog/why-tokenize-domains/
  - /zh-CN/blog/the-curve-finance-dns-hijack/
  - /zh-CN/blog/what-are-xstocks/
  - /zh-CN/blog/why-a-dao-should-control-the-main-domain/
relatedTopics:
  - /zh-CN/topics/domain-tokenization/
  - /zh-CN/topics/web3-foundations/
relatedSeries:
  - /zh-CN/series/domain-flipping-skills/
  - /zh-CN/series/domain-apocalypse/
relatedGlossary:
  - /zh-CN/glossary/web3/
  - /zh-CN/glossary/composability/
  - /zh-CN/glossary/collateral/
  - /zh-CN/glossary/icann/
  - /zh-CN/glossary/tokenize/
---

**协议资产** 是一种数字资产，在区块链协议或去中心化系统中充当基本构建模块或基础设施组件。这些资产通常具有超出简单价值存储的实用性，提供其他应用程序和服务可以构建在其上的功能。[代币化](/zh-CN/glossary/tokenize/) 域名通过充当 [Web3](/zh-CN/glossary/web3/) 应用程序的身份层、命名空间系统和集成点来发挥协议资产的作用。它们为更广泛的生态系统提供实用性，同时可以与其他协议 [组合](/zh-CN/glossary/composability/)。例如，一个 代币化 域名可能在 DeFi 中充当 [抵押品](/zh-CN/glossary/collateral/)、在社交协议中充当身份，并在 [钱包](/zh-CN/glossary/wallet/) 系统中充当命名——使其成为一个多功能协议资产。