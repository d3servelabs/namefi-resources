---
title: 底价
date: '2026-06-22'
language: zh-CN
tags: ['glossary']
authors: ['namefiteam']
editors: ['victor-zhou']
translators: ['fenwei-bian']
description: 卖家设定的最低可接受价格，通常保密，低于此价格的出价或竞拍将不予成交。
keywords: ['底价', '最低出价', '保留价', '底线价格', '域名拍卖底价']
level: 1
sources:
  - https://www.investopedia.com/terms/r/reserveprice.asp
  - https://www.sedo.com/
relatedArticles:
  - /zh-CN/blog/how-to-win-domain-auctions/
  - /zh-CN/blog/working-with-domain-brokers/
  - /zh-CN/blog/domain-pricing-psychology-buy-now-vs-make-offer/
  - /zh-CN/blog/end-user-vs-reseller-domain-pricing/
  - /zh-CN/blog/how-to-sell-domains-for-profit/
relatedTopics:
  - /zh-CN/topics/domain-investing/
  - /zh-CN/topics/domain-basics/
relatedSeries:
  - /zh-CN/series/domain-flipping-skills/
  - /zh-CN/series/domain-apocalypse/
relatedGlossary:
  - /zh-CN/glossary/domain-broker/
  - /zh-CN/glossary/make-offer/
  - /zh-CN/glossary/registrar/
  - /zh-CN/glossary/icann/
  - /zh-CN/glossary/tld/
---

**底价**是域名卖家设定的保密最低售价，低于此价格的[拍卖](/zh-CN/glossary/auction/)竞价或协商报价均不会促成交易。在域名拍卖中，若竞价在底价达成前结束，该域名不会被授予最高出价者——仅以流拍告终。GoDaddy Auctions、Sedo、NameJet 等主流拍卖平台均允许卖家设置隐藏底价，竞拍者能看到当前最高出价，但无法得知是否已达到底线。底价设置过高会导致反复流拍并损害信誉；底价设置过低则有低价成交的风险。在私下[议价](/zh-CN/glossary/make-offer/)挂牌中，底价充当卖家谈判中的内心底线，通常告知[域名经纪人](/zh-CN/glossary/domain-broker/)，但不对买家披露。
