---
title: 待删除（域名释放）
date: '2026-06-22'
language: zh-CN
priority: P1
tags: ['glossary']
authors: ['namefiteam']
description: 未续期域名被释放重新公开注册前的最后状态。
keywords: ['待删除', '域名释放', '抢注', '已到期域名', '释放']
level: 1
sources:
  - https://www.icann.org/resources/pages/epp-status-codes-2014-06-16-en
relatedArticles:
  - /zh-CN/blog/expired-domains-and-the-drop-cycle/
  - /zh-CN/blog/domain-backorders-and-drop-catching/
  - /zh-CN/blog/how-to-win-domain-auctions/
  - /zh-CN/blog/hand-registering-domains-to-flip/
  - /zh-CN/blog/when-to-drop-a-domain/
relatedTopics:
  - /zh-CN/topics/domain-investing/
  - /zh-CN/topics/domain-tokenization/
relatedSeries:
  - /zh-CN/series/domain-flipping-skills/
  - /zh-CN/series/domain-apocalypse/
relatedGlossary:
  - /zh-CN/glossary/registry/
  - /zh-CN/glossary/icann/
  - /zh-CN/glossary/backorder/
  - /zh-CN/glossary/registrar/
  - /zh-CN/glossary/dns/
---

**待删除**是失效域名生命周期的最后状态：[赎回期](/zh-CN/glossary/redemption-period/) 结束后仍未恢复，[注册局](/zh-CN/glossary/registry/) 将域名标记为 `pendingDelete` 约五天，在此期间既无法续期也无法转移。该窗口结束后，域名**被释放**——从系统中清除并重新对公众开放，可按先到先得的原则重新注册。正是在这个时刻，[预订单](/zh-CN/glossary/backorder/) 和抢注服务竞相争夺抢手域名。待删除纯粹是传统 [DNS（域名系统）](/zh-CN/glossary/dns/) 体系中[注册局](/zh-CN/glossary/registry/) 层面的状态；它在链上所有权层没有对应物，这也是为什么保持代币化域名的底层注册及时续期仍然至关重要。

*来源：ICANN EPP 状态码。*
