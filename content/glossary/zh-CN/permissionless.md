---
title: 无许可
date: '2025-06-30'
language: zh-CN
tags: ["glossary"]
authors: ["namefiteam"]
editors: ['victor-zhou']
translators: ['fenwei-bian']
description: 区块链和域名管理中的“无许可”是什么意思？
keywords: ["无许可","去中心化","抗审查","开放访问","区块链"]
relatedArticles:
  - /zh-CN/blog/why-tokenize-domains/
  - /zh-CN/blog/what-are-tokenized-domains/
  - /zh-CN/blog/tokenized-domain-use-cases-2026/
  - /zh-CN/blog/the-curve-finance-dns-hijack/
  - /zh-CN/blog/what-are-stablecoins/
relatedTopics:
  - /zh-CN/topics/domain-tokenization/
  - /zh-CN/topics/domain-security/
relatedSeries:
  - /zh-CN/series/domain-apocalypse/
  - /zh-CN/series/domain-flipping-skills/
relatedGlossary:
  - /zh-CN/glossary/web3/
  - /zh-CN/glossary/registrar/
  - /zh-CN/glossary/tokenized-domain/
  - /zh-CN/glossary/blockchain/
  - /zh-CN/glossary/tokenize/
---


**无许可 (Permissionless)** 指的是允许任何人参与而无需中央机构批准的系统。在[区块链](/zh-CN/glossary/blockchain/)环境中，无许可网络允许用户进行交易、构建应用程序或与协议交互，而无需守门人。传统的域名管理需要 [注册商](/zh-CN/glossary/registrar/) 的许可并遵循其政策，但 Namefi 上的[代币化域名](/zh-CN/glossary/tokenized-domain/)支持无许可的转移和集成。一旦您拥有一个代币化域名，只要底层的区块链网络对您是可访问的，您就可以交易它、在 DeFi 协议中使用它或围绕它构建应用程序，而无需寻求中介机构的批准。