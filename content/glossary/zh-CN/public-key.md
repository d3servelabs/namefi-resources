...
**公钥**（public key）是[区块链](/zh-CN/glossary/blockchain/)账户密码密钥对中可公开分享的一半。公钥本身——或由其派生的地址——可安全地公开发布：其他人向你发送代币或以你的名义调用智能合约时，都需要用到它。公钥通过单向椭圆曲线运算从[私钥](/zh-CN/glossary/private-key/)派生而来，因此公开分享公钥不会暴露授权交易的私密信息。将数字签名与公钥进行核验，可证明某条消息由对应私钥的持有者签署，这也是网络确认交易确实获得授权的方式。
