---
title: 卸売価格
date: '2026-06-22'
language: ja
tags: ['glossary']
authors: ['namefiteam']
description: ドメイン投資家間で成立する最低価格——エンドユーザーが支払う小売価格を下回り、迅速に資金化するために受け入れられる価格。
keywords: ['卸売価格', '投資家間価格', 'ドメイン卸売', '清算価格', 'ドメイン投資家価格']
also_known_as: ['投資家間価格']
level: 1
sources:
  - https://www.investopedia.com/terms/w/wholesaling.asp
  - https://www.namebio.com/
relatedArticles:
  - /ja/blog/end-user-vs-reseller-domain-pricing/
  - /ja/blog/why-are-io-domains-expensive/
  - /ja/blog/how-to-value-a-domain-name/
  - /ja/blog/domain-pricing-psychology-buy-now-vs-make-offer/
  - /ja/blog/domain-renewal-costs-and-sell-through-rate/
relatedTopics:
  - /ja/topics/domain-investing/
  - /ja/topics/choosing-a-tld/
relatedSeries:
  - /ja/series/domain-flipping-skills/
  - /ja/series/domain-investor-field-guide/
relatedGlossary:
  - /ja/glossary/retail-pricing/
  - /ja/glossary/end-user/
  - /ja/glossary/registrar/
  - /ja/glossary/registry/
  - /ja/glossary/domain-appraisal/
---

**卸売価格**（ホールセール・プライシング）とは、ドメイン投資家同士の間で成立する最低取引価格のことで、別名「投資家間価格」とも呼ばれる。一般的に、[エンドユーザー](/ja/glossary/end-user/)が小売で支払う価格の20〜40%程度が相場となる。売り手が資金を早急に回収したい場合、この割引を受け入れることになるが、それは買い手が転売リスクや必要な投資利回りを価格に織り込んでいる投資家であり、そのドメインを実際に必要としているわけではないためだ。卸売取引はスピードが命であり、ポートフォリオのバルクセール、投資家フォーラム、ブローカーネットワークなどでこの価格帯の取引が活発に行われる。ただし、[小売価格](/ja/glossary/retail-pricing/)と比べると得られる収益は大幅に少ない。卸売価格のフロアを把握することは[ドメイン査定](/ja/glossary/domain-appraisal/)において不可欠であり、エンドユーザー向けの希望価格とは別に、現実的な即時売却価値を算出する基準となる。
