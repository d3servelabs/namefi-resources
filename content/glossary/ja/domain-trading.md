---
title: ドメイン売買
date: '2025-06-30'
language: ja
priority: P1
tags: ['glossary']
authors: ['namefiteam']
description: 価格差から利益を得るためにドメイン名を売買すること。短期転売から管理されたポートフォリオでの長期保有まで幅広い手法がある。
keywords: ['ドメイン売買', 'ドメイン投資', 'デジタル不動産', 'セカンダリーマーケット', '流動性']
level: 1
sources:
  - https://www.namebio.com/
relatedArticles:
  - /ja/blog/onchain-domain-flipping/
  - /ja/blog/end-user-vs-reseller-domain-pricing/
  - /ja/blog/cybersquatting-vs-domaining-udrp-acpa/
  - /ja/blog/taxes-and-accounting-for-domain-investors/
  - /ja/blog/why-tokenize-domains/
relatedTopics:
  - /ja/topics/domain-investing/
  - /ja/topics/domain-tokenization/
relatedSeries:
  - /ja/series/domain-flipping-skills/
  - /ja/series/domain-apocalypse/
relatedGlossary:
  - /ja/glossary/domain-liquidity/
  - /ja/glossary/tokenize/
  - /ja/glossary/domaining/
  - /ja/glossary/on-chain/
  - /ja/glossary/escrow/
---

**ドメイン売買**とは、株式や不動産と同様に、ドメイン名を投資資産として売買することを指す。従来のドメイン売買では、長期にわたる移管手続き、[エスクロー](/ja/glossary/escrow/)サービスの利用、そして[流動性](/ja/glossary/domain-liquidity/)の低さという課題があった。一方、[トークン化](/ja/glossary/tokenize/)されたドメインは NFT として[マーケットプレイス](/ja/glossary/marketplace/)上で取引でき、[オンチェーン](/ja/glossary/on-chain/)での移転や価格履歴の透明性が確保される。
