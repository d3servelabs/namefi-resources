---
title: 批发定价
date: '2026-06-22'
language: zh
tags: ['glossary']
authors: ['namefiteam']
description: 域名投资者之间交易的底线价格——为快速变现而接受的低于零售价的折让。
keywords: ['批发定价', '投资者定价', '域名批发', '清仓价', '域名投资者定价']
also_known_as: ['投资者定价']
level: 1
sources:
  - https://www.investopedia.com/terms/w/wholesaling.asp
  - https://www.namebio.com/
relatedArticles:
  - /zh/blog/end-user-vs-reseller-domain-pricing/
  - /zh/blog/why-are-io-domains-expensive/
  - /zh/blog/how-to-value-a-domain-name/
  - /zh/blog/domain-pricing-psychology-buy-now-vs-make-offer/
  - /zh/blog/domain-renewal-costs-and-sell-through-rate/
relatedTopics:
  - /zh/topics/domain-investing/
  - /zh/topics/choosing-a-tld/
relatedSeries:
  - /zh/series/domain-flipping-skills/
  - /zh/series/domain-investor-field-guide/
relatedGlossary:
  - /zh/glossary/retail-pricing/
  - /zh/glossary/end-user/
  - /zh/glossary/registrar/
  - /zh/glossary/registry/
  - /zh/glossary/domain-appraisal/
---

**批发定价**是[域名投资](/zh/glossary/domaining/)者之间交易的底线价格：一位投资者愿意卖给另一位投资者的价格，通常仅为[终端用户](/zh/glossary/end-user/)零售价的 20–40%。急于回笼资金的卖家接受这一折扣，因为买家作为投资者，其出价已计入自身的转售风险和所需回报，而非真正需要这个域名。批发交易成交迅速——批量投资组合出售、投资者论坛和经纪网络均在这一价位运作——但收益远低于[零售定价](/zh/glossary/retail-pricing/)。理解批发底线对[域名估值](/zh/glossary/domain-appraisal/)至关重要：它代表了现实的快速变现价值，有别于面向终端用户的理想定价。
