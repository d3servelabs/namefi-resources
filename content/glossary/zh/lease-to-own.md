---
title: 租购
date: '2026-06-22'
language: zh
tags: ['glossary']
authors: ['namefiteam']
description: 通过定期分期付款逐步取得域名完整所有权的收购方式，是"租转买"模式的近亲。
keywords: ['租购', '域名收购', '分期付款', '域名融资', '租转买']
level: 1
sources:
  - https://www.investopedia.com/terms/l/lease-option.asp
relatedArticles:
  - /zh/blog/domain-portfolio-management/
  - /zh/blog/tokenize-your-com-to-flip-it/
  - /zh/blog/selling-domains-as-nfts/
  - /zh/blog/domain-for-sale-landing-pages/
  - /zh/blog/marketing-your-domains-for-sale/
relatedTopics:
  - /zh/topics/domain-investing/
  - /zh/topics/domain-tokenization/
relatedSeries:
  - /zh/series/domain-flipping-skills/
  - /zh/series/domain-apocalypse/
relatedGlossary:
  - /zh/glossary/rent-to-own/
  - /zh/glossary/domain-financing/
  - /zh/glossary/urs/
  - /zh/glossary/escrow/
  - /zh/glossary/registrar/
---

**租购**（lease-to-own）是一种域名收购结构：买方按约定周期——每月或每年——定期支付款项，这些款项计入总购买价款，待最后一期款项结清后方可完成所有权转让。它与[租转买](/zh/glossary/rent-to-own/)并列，是将高额前期成本分摊至较长时间跨度、同时从第一天起即可使用域名的常见方式。在付款期间，卖方通常保留法律所有权，若买方违约，已支付的款项将面临无法追偿的风险。[域名融资](/zh/glossary/domain-financing/)是对这一概念的进一步推广，而在高价值交易中，通常会借助[第三方托管](/zh/glossary/escrow/)服务来保管款项并监督里程碑的执行。
