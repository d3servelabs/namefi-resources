---
title: 售出率
date: '2026-06-22'
language: zh
tags: ['glossary']
authors: ['namefiteam']
description: 在特定周期内投资组合中实际售出域名的比例，是衡量投资者流动性的关键指标。
keywords: ['售出率', 'STR', '域名流动性', '投资组合表现', '域名销售速度']
also_known_as: ['STR']
level: 1
sources:
  - https://www.namebio.com/
  - https://www.investopedia.com/terms/s/sell-through-rate.asp
relatedArticles:
  - /zh/blog/domain-renewal-costs-and-sell-through-rate/
  - /zh/blog/domain-portfolio-management/
  - /zh/blog/taxes-and-accounting-for-domain-investors/
  - /zh/blog/when-to-drop-a-domain/
  - /zh/blog/domain-flipping/
relatedTopics:
  - /zh/topics/domain-investing/
  - /zh/topics/choosing-a-tld/
relatedSeries:
  - /zh/series/domain-flipping-skills/
  - /zh/series/domain-investor-field-guide/
relatedGlossary:
  - /zh/glossary/holding-cost/
  - /zh/glossary/domain-liquidity/
  - /zh/glossary/domain-portfolio/
  - /zh/glossary/tld/
  - /zh/glossary/aftermarket/
---

**售出率**（STR）是指[域名投资组合](/zh/glossary/domain-portfolio/)中在特定周期内（通常为一个自然年）实际售出的域名比例，是衡量投资者整体账户[流动性](/zh/glossary/domain-liquidity/)的实用指标。即便是大型、声誉良好的投资组合，年售出率也很少超过几个百分点；一个精心筛选的 [.com](/zh/tld/com/) 投资组合实现 2–3% 的年售出率已属健康水平，这意味着大多数域名需要持有多年才能找到买家。售出率偏低而持有成本偏高，通常意味着需要对投资组合进行清理——在续费费用侵蚀回报之前剔除边际域名。将售出率与平均售价、持有周期结合分析，可以更清晰地评估投入[二级市场](/zh/glossary/aftermarket/)资本的实际回报。
