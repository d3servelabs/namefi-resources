...
Una auditoría examina el código en la cadena: las bóvedas, la lógica de tokens, los controles de acceso. No dice nada sobre el portátil que un desarrollador dejó con la sesión iniciada, los registros DNS que apuntan tu navegador a algún lugar, la CDN que se encuentra frente al sitio, o el JavaScript que tu navegador realmente descarga y ejecuta cuando visitas la dApp. Esos viven en *Web2* — en cuentas en la nube, claves API e infraestructura de dominio — y son igual de críticos que el Solidity.
