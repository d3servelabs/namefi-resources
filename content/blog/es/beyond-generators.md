...
Ninguno de estos pasos es difícil por sí solo. En conjunto, son doce acciones manuales repartidas entre el panel de un registrador, un panel de DNS y tu calendario, para una decisión que supuestamente la IA ya te ayudó a tomar con una sola indicación. El paso 12 no es un mito: ICANN explica que [se necesita un Auth-Code para transferir un dominio de un registrador a otro](https://www.icann.org/resources/pages/name-holder-faqs-2017-10-10-en#:~:text=An%20Auth%2DCode%20is%20required%20for%20a%20domain%20holder%20to%20transfer%20a%20domain%20name%20from%20one%20registrar%20to%20another). Su Política de Transferencia vigente también permite que un registrador deniegue una transferencia dentro de los 60 días posteriores a la creación del dominio o a una transferencia anterior entre registradores, y exige un bloqueo de 60 días tras determinados Cambios de Registrante, salvo que el registrador hubiera ofrecido la posibilidad de no aplicar ese bloqueo y el registrante la hubiera elegido ([secciones 3.7.5–3.8.5 y II.C.2](https://www.icann.org/en/contracted-parties/accredited-registrars/transfer-policy-01-06-2016-en#:~:text=3.7.5%20The%20transfer%20was%20requested%20within%2060%20days)). Estas reglas vigentes no han sido sustituidas de forma general por un bloqueo universal de 30 días. Una persona todavía tiene que saber qué regla se aplica y actuar manualmente.
...
Hemos desarrollado los ejemplos prácticos completos en otros artículos para no repetirlos aquí. [Cómo comprar un dominio con lenguaje natural](/es/blog/nl-domain-purchase/) recorre una conversación anotada desde una indicación en lenguaje natural hasta un dominio registrado y configurado con DNS, con el mecanismo de los peldaños 3 y 4 en detalle. [Cómo registrar un dominio con tu agente de IA en Namefi](/es/blog/ai-agent-register/) es la guía de configuración canónica para los principales clientes de agentes, con el flujo universal de cinco pasos: obtener credenciales, conectarse, buscar y consultar precios, registrar y configurar DNS. El peldaño 5 es la parte más nueva: las herramientas documentadas de Namefi permiten que un agente active o desactive la renovación automática y edite los registros DNS meses después mediante la misma interfaz con la que registró el dominio, sin iniciar sesión en un panel separado. El catálogo publicado no documenta actualmente una operación para iniciar una transferencia entre registradores, por lo que esa parte de la automatización integral del ciclo de vida aún requiere una vía aparte.
...
En parte, pero no para todas las operaciones incluidas en la definición de ese peldaño. La referencia de API publicada de Namefi documenta la lectura y escritura programáticas de registros DNS y un interruptor de renovación automática, por lo que un agente puede encargarse de una parte importante de la gestión continua después del registro. Actualmente no documenta una operación para iniciar una transferencia entre registradores ni para actualizar todos los campos de contacto del registrante. Tampoco se documenta públicamente una primitiva de límite de gasto en el servidor: esa salvaguarda vive actualmente en el cliente MCP o la capa de políticas que configures a su alrededor.
...
Si tu agente ya puede redactar el código y elegir el nombre, no hay motivo para que comprobar, comprar, configurar DNS y gestionar la renovación vuelvan a depender de que hagas clic en un panel. [Namefi](https://namefi.io) expone la búsqueda de dominios, el registro, la gestión de DNS y los controles de renovación como herramientas que un agente compatible con MCP puede llamar directamente, autenticado con una clave de API o una firma de billetera, de modo que la escalera no tiene por qué detenerse en un nombre. Las transferencias entre registradores siguen fuera del catálogo de herramientas documentado actualmente.
...
- ICANN — [Transfer Policy](https://www.icann.org/en/contracted-parties/accredited-registrars/transfer-policy-01-06-2016-en) · [Registrant transfer FAQ](https://www.icann.org/resources/pages/name-holder-faqs-2017-10-10-en) — el requisito del Auth-Code y las restricciones vigentes de 60 días para las transferencias.
