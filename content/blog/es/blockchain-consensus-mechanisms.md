...
**Proof of History (PoH)**, que Solana usa junto con PoS, no es un mecanismo de consenso independiente, sino un reloj criptográfico. Inserta marcas de tiempo verificables directamente en la cadena aplicando repetidamente una función hash a «los datos de los estados generados previamente», creando una secuencia que prueba cuánto tiempo transcurrió entre eventos sin que los validadores tengan que comunicarse sobre el tiempo ([Solana: Proof of History](https://solana.com/news/proof-of-history#:~:text=inserting%20data%20into%20the%20sequence%20by%20appending%20the%20hash%20of%20the%20data%20of%20the%20previously%20generated%20states)). Al hacer que el propio tiempo sea verificable, permite a los validadores procesar y verificar transacciones en paralelo, reduciendo la sobrecarga de comunicación que normalmente ralentiza el consenso.

**Proof of Authority (PoA)** elimina por completo el coste económico y computacional en favor de la reputación. Un conjunto fijo de validadores conocidos e identificados —«corporaciones conocidas» o personas examinadas— recibe la confianza para producir bloques; las conductas indebidas se disuaden mediante responsabilidad reputacional y legal, en vez de mediante staking recortado o cómputo desperdiciado ([ethereum.org: Proof-of-authority](https://ethereum.org/en/developers/docs/consensus-mechanisms/poa/#:~:text=The%20reputation%20in%20this%20context%20is%20not%20a%20quantified%20thing)). Sacrifica descentralización por velocidad y bajo coste, por lo que se emplea principalmente en cadenas privadas, redes de prueba y redes de desarrollo local, y no en redes públicas y adversariales.
...
Los mecanismos de consenso son la base invisible de todo [dominio tokenizado](/en/blog/what-are-tokenized-domains/). Cuando un dominio `.com`, `.ai` o `.io` se acuña como un [NFT](/en/glossary/nft/), el registro de quién posee ese token —y de cada transferencia, venta o renovación posterior— solo es tan fiable como el mecanismo de consenso que protege la cadena en la que reside. Un NFT de dominio acuñado en [Ethereum](/en/glossary/ethereum/) hereda la finalidad rápida y de bajo coste de PoS, así como su conjunto de validadores, que se cuenta por cientos de miles; el mismo activo en una cadena PoW heredaría una finalidad probabilística y costes de transacción mucho mayores. Comprender qué mecanismo sustenta una cadena y qué significan realmente sus garantías de resistencia a Sybil y finalidad forma parte de la evaluación de cualquier activo on-chain, incluidos los dominios tokenizados.
