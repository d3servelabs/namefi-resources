...
Es posible que hayas escuchado frases como "[dominio tokenizado](/es/glossary/tokenized-domain/)", "[tokenizar](/es/glossary/tokenize/) un dominio" o "tokenización de dominios" y te hayas preguntado qué significan realmente. ¿Es un dominio tokenizado un nuevo tipo de dominio? ¿Un nombre exclusivo de [blockchain](/es/glossary/blockchain/)? ¿Un reemplazo para `.com`? Y, en primer lugar, ¿qué significa *tokenizar* un dominio?
...
> Un dominio tokenizado es un dominio con **dos capas de propiedad sincronizadas**: el [registro](/es/glossary/registry/) tradicional de [DNS](/es/glossary/dns/), *y* un token [on-chain](/es/glossary/on-chain/) que lo refleja. **Tokenizar un dominio** consiste en agregar esa segunda capa on-chain a un nombre de dominio existente o recién registrado.
...
|---|---|---|
| DNS / Registro | El registro oficial en el [registrador](/es/glossary/registrar/) y en el registro | Registradores acreditados por la ICANN |
| Token on-chain | Un NFT en tu [billetera](/es/glossary/wallet/) que representa la propiedad | Un [contrato inteligente](/es/glossary/smart-contract/) en una blockchain pública |
...
Debido a que el activo subyacente es un dominio DNS real, los dominios tokenizados siguen sujetos a renovación, a las políticas de la ICANN, a las disputas [UDRP](/es/glossary/udrp/) y a la ley aplicable. El token refleja la propiedad; no exime al dominio de las reglas del mundo real.
...
1. **Registro**: Se registra (o transfiere) un dominio DNS real a través de un registrador acreditado.
2. **[Acuñación](/es/glossary/minting/) (Minting)**: Como parte de la tokenización del dominio, se acuña en tu billetera un NFT que representa dicho dominio.
...
**Enfoque:** Tokenizar dominios ICANN reales (`.com`, `.xyz`, `.io`, `.art` y muchos más) como NFT manteniendo la capa de DNS completamente funcional. Ambas capas se mantienen sincronizadas a través de registradores acreditados.

**Lo que distingue a Namefi:** Namefi fue la **primera plataforma en tokenizar dominios ICANN reales en la red principal (mainnet) de [Ethereum](/es/glossary/ethereum/), y la primera en hacerlo en Base**. Debido a que los dominios tokenizados por Namefi viven en Ethereum y Base, se integran de manera natural con la **mayoría de los principales mercados de NFT y protocolos de préstamos** (OpenSea, Blur, NFTfi y otros) gracias al profundo y maduro ecosistema DeFi de Ethereum. Otras plataformas han tomado sus propias decisiones sobre las redes que mejor se adaptan a sus objetivos; sin embargo, Ethereum y Base brindan a los usuarios de Namefi la compatibilidad inmediata más amplia con las herramientas de NFT y DeFi existentes en la actualidad.
...
- **Un espacio de nombres diferente por diseño.** Los dominios Web3 (`.eth`, `.crypto`, `.x`, `.nft` y los TLD creados por los usuarios) viven intencionalmente fuera de la raíz de la ICANN, lo que les permite iterar rápidamente y experimentar con nuevos modelos de nomenclatura. La contrapartida es que se sitúan junto a la jerarquía de DNS tradicional en lugar de estar dentro de ella.
