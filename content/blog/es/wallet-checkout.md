...
Dos detalles son importantes para la seguridad: **las firmas caducan después de 300 segundos y los nonces son de un solo uso.** Una firma capturada en tránsito, o una solicitud repetida más tarde, no funciona: ya habrá caducado o su nonce ya se habrá consumido. La documentación de Namefi también especifica que las definiciones de tipo EIP-712 vigentes deben obtenerse de sus endpoints `/v-next/eip712/` en el momento de la solicitud, no codificarse de forma fija en una integración, porque el esquema exacto al que debe ajustarse una firma puede cambiar.
...
No en la vía manual EIP-712, donde las firmas caducan después de 300 segundos y cada nonce solo puede usarse una vez. La autorización EIP-3009 del flujo x402 también está limitada por su intervalo `validAfter`/`validBefore`, por lo que una firma capturada tiene una vida útil corta y finita, en lugar de mantenerse indefinidamente como lo haría un token de tarjeta guardada.
...
Limita un conjunto de riesgos distinto, en vez de eliminar el riesgo por completo. No hay una credencial permanente que un sistema comprometido pueda reutilizar indefinidamente, y cada pago es una firma nueva, con límite de tiempo y de un solo uso; pero el protocolo en sí no limita cuánto puede autorizar una solicitud firmada, por lo que el tope práctico de lo que un agente puede gastar sigue viniendo de cuánto financies la billetera y de cualquier política de aprobación adicional (como una multifirma) que sitúes delante de ella.
