...
3. **Verrouillez ce qui peut l'être.** Les verrous de bureau d'enregistrement (registrar-lock / `clientTransferProhibited`), l'accès restreint aux codes EPP/auth, et les verrous au niveau du registre pour les domaines de grande valeur existent précisément pour empêcher les modifications non autorisées des serveurs de noms et les transferts. Ils sont peu coûteux. L'inconvénient de les ignorer, c'est votre marque sur un diaporama de webcam.
4. **DNSSEC augmente le coût.** Cela n'aurait pas suffi à empêcher une prise de contrôle du compte du bureau d'enregistrement en soi, mais les zones signées et la surveillance DNS rendent la falsification silencieuse plus difficile à réaliser sans être détecté.
