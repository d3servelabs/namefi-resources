...
Mais un utilisateur n'interagit presque jamais directement avec un contrat. Il se rend sur un site web. Il tape `curve.fi`, son navigateur résout ce nom en [adresse IP](/fr/glossary/ip-address/), charge une page, et cette page indique à son [portefeuille](/fr/glossary/wallet/) quoi signer. Chacune de ces étapes se produit *avant* qu'une seule ligne de Solidity audité ne s'exécute — et chacune d'elles réside dans une infrastructure que l'audit n'a jamais couverte.
...
4. **Ayez une solution de repli propre.** L'atout salutaire de Curve était `curve.exchange` sur une infrastructure séparée. Une deuxième voie d'accès — un domaine différent, un fournisseur DNS différent, un miroir [IPFS](/fr/glossary/ipfs/) ou ENS — vous donne un endroit vers lequel rediriger les utilisateurs quand votre nom principal est empoisonné.
