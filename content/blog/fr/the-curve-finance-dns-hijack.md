...
C'est la première chose à comprendre sur ce qui est arrivé à Curve Finance le 9 août 2022, et c'est ce qui continue de déconcerter les ingénieurs en sécurité des années plus tard. Le code on-chain de Curve — l'automated market maker audité et éprouvé qui détenait des milliards en stablecoins — n'a jamais été touché. Aucun bug de réentrance. Aucune manipulation d'oracle. Aucun exploit de flash loan. La blockchain a fait exactement ce qu'elle était censée faire.
...
Mais un utilisateur n'interagit presque jamais directement avec un contrat. Il se rend sur un site web. Il tape `curve.fi`, son navigateur résout ce nom en adresse IP, charge une page, et cette page indique à son portefeuille quoi signer. Chacune de ces étapes se produit *avant* qu'une seule ligne de Solidity audité ne s'exécute — et chacune d'elles réside dans une infrastructure que l'audit n'a jamais couverte.
...
4. **Ayez une solution de repli propre.** L'atout salutaire de Curve était `curve.exchange` sur une infrastructure séparée. Une deuxième voie d'accès — un domaine différent, un fournisseur DNS différent, un miroir IPFS ou ENS — vous donne un endroit vers lequel rediriger les utilisateurs quand votre nom principal est empoisonné.
