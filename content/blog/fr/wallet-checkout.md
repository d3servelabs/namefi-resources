...
Deux éléments sont essentiels pour la sécurité : **les signatures expirent après 300 secondes et les nonces sont à usage unique.** Une signature interceptée pendant son transit ou une requête réémise plus tard ne fonctionne pas : soit elle a déjà expiré, soit son nonce a déjà été consommé. La documentation de Namefi précise également que les définitions de types EIP-712 en vigueur doivent être récupérées depuis ses points de terminaison `/v-next/eip712/` au moment de la requête et non codées en dur dans une intégration, car le schéma exact auquel une signature doit correspondre peut évoluer.
...
Pas avec le parcours manuel EIP-712, dans lequel les signatures expirent après 300 secondes et chaque nonce ne peut être utilisé qu'une seule fois. Dans le parcours x402, l'autorisation EIP-3009 est, elle aussi, limitée par sa fenêtre `validAfter` / `validBefore` : une signature interceptée a donc une durée de vie courte et finie, contrairement au jeton d'une carte enregistrée qui reste valide sans limite de temps prédéfinie.
...
Il limite un autre ensemble de risques plutôt qu'il ne supprime tout risque. Aucun identifiant permanent ne peut être réutilisé indéfiniment par un système compromis, et chaque paiement repose sur une nouvelle signature à usage unique et limitée dans le temps. Le protocole ne plafonne toutefois pas le montant qu'une requête signée peut autoriser ; la limite pratique des dépenses d'un agent dépend donc toujours de la somme dont vous approvisionnez le portefeuille et des règles d'approbation supplémentaires, comme une Multi-signature (Multi-sig), que vous lui imposez.
