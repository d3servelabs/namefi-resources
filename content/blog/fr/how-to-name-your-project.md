---
title: "Comment nommer votre projet : le guide terrain du fondateur"
date: '2026-06-13'
language: fr
...
keywords: ['comment nommer sa startup', 'comment nommer son projet', 'comment nommer une entreprise', 'nommer un produit', 'idées de nom de marque', 'choisir un nom commercial', 'sélection de nom de domaine', 'recherche de marque déposée', 'domaine brandable', 'image de marque startup', 'nom de marque prononçable', 'fluidité de traitement', 'guide de dénomination de marque', 'choisir un nom pour son projet', 'vérifier la disponibilité d''un domaine']
relatedArticles:
...
Nommer son projet est la première décision produit que la plupart des fondateurs ratent.
...
1. **Être appropriable** — légalement défendable et techniquement disponible, pour pouvoir devenir réellement *le vôtre*.
2. **Être facile à commercialiser** — facile à dire, à épeler, à retenir et à transmettre, pour que le nom fasse votre marketing à votre place et que d'autres puissent le diffuser sans effort.
...
- **Bases de données de marques.** Recherchez dans le [système de recherche de marques de l'USPTO](https://www.uspto.gov/trademarks/search) pour les États-Unis, la [base de données de l'EUIPO](https://www.euipo.europa.eu/en/trade-marks/before-applying) pour l'UE, et la [base de données mondiale des marques de l'OMPI](https://branddb.wipo.int/) pour un balayage international. Vous ne faites pas une analyse juridique complète — vous repérez les collisions évidentes gratuitement.
- **Domaines.** Le `.com` ancre encore la crédibilité pour la plupart des projets. Sur [Namefi](https://namefi.io) vous pouvez vérifier la disponibilité, voir **depuis combien de temps un `.com` est enregistré** (un nom détenu en continu depuis 2009 envoie un signal très différent d'un nom enregistré le mois dernier), et voir **sur combien d'extensions [TLD](/fr/glossary/tld/) le nom est déjà enregistré**. Un nom pris sur 30 extensions est un territoire disputé ; un nom libre presque partout est une route dégagée. Utilisez cette distribution comme jauge rapide de « à quel point ce mot est-il encombré ».
- **Handles.** Vérifiez l'organisation [GitHub](https://github.com), ainsi que X, Reddit, LinkedIn et la plateforme sur laquelle se trouvent réellement vos utilisateurs. Des handles cohérents sur le web (« uniformité des handles ») valent étonnamment beaucoup pour un projet en démarrage — et ils fonctionnent selon le principe du premier arrivé, premier servi.
...
![Illustration éditoriale de construction de noms : tuiles de mots combinées en composés, préfixes et suffixes, et fautes d'orthographe créatives](../../assets/how-to-name-your-project-03-build-candidates.jpg)
...
- **Préfixes et suffixes** — ajoutez `-ly`, `-ify`, `-io`, `-ai`, `-hq` : *Spotify*, *Shopify*, *Calendly*.
- **Fautes d'orthographe** — transformez un mot familier pour que le domaine soit appropriable : *Flickr*, *Tumblr*, *Lyft*, et *Google* lui-même (de « googol »).
...
![Illustration éditoriale d'un mot de nom abstrait qui se résout en une simple forme de logo iconique, comme une feuille ou une flèche](../../assets/how-to-name-your-project-04-make-it-concrete.jpg)
...
![Illustration éditoriale d'une carte de nom vérifiée par rapport à plusieurs langues et scripts sur une carte du monde, avec l'une signalée pour une mauvaise signification](../../assets/how-to-name-your-project-05-travel-test.jpg)
...
Une mise en garde sur les exemples célèbres, cependant : le plus répété est faux. L'histoire selon laquelle la Chevrolet **Nova** a échoué en Amérique latine parce que *« no va »* signifie « ne va pas » est une [légende urbaine — *nova* et *no va* sont accentuées et prononcées différemment, et la voiture s'est en fait bien vendue](https://www.snopes.com/fact-check/chevrolet-nova-name-spanish/#:~:text=pronounced). Ne construisez pas votre philosophie de dénomination sur des mythes.

Les *vrais* cas sont moins célèbres mais instructifs. Mitsubishi a vendu son SUV Pajero sous le nom de [**Montero** sur les marchés hispanophones parce que « pajero » est un argot vulgaire en espagnol](https://en.wikipedia.org/wiki/Mitsubishi_Pajero#:~:text=Montero). La leçon reste valable : vérifiez avant de lancer, pas après avoir imprimé les boîtes. Une conversation de deux minutes avec un locuteur natif est l'assurance la moins chère que vous aurez jamais.

## Partie 6 — Le balayage de recherche pré-lancement

![Illustration de tableau de bord éditorial des vérifications de nom pré-lancement sur la recherche web, les bases de données de marques, le domaine, GitHub et les handles sociaux, chacun avec un indicateur de statut](../../assets/how-to-name-your-project-06-launch-sweep.jpg)
...
1. **Recherche web** — la première page est-elle libre de concurrents ?
2. **Marque** — [USPTO](https://www.uspto.gov/trademarks/search), [EUIPO](https://www.euipo.europa.eu/en/trade-marks/before-applying) et [Base de données mondiale des marques OMPI](https://branddb.wipo.int/) pour votre catégorie.
3. **Domaine** — le `.com` est-il disponible ou acquérable ? Sur [Namefi](https://namefi.io), vérifiez **l'âge d'enregistrement du `.com`** et le **nombre d'extensions TLD déjà enregistrées** pour évaluer à quel point le nom est disputé.
4. **GitHub** — l'organisation/le handle est-il libre ? (Critique pour tout ce qui est orienté développeurs.)
5. **Handles sociaux** — X, Reddit, LinkedIn, Instagram, YouTube — tout ce qui compte pour votre audience, idéalement tous correspondants.
6. **Stores d'applications**, si vous lancez une application.
...
- USPTO — [À propos de la contrefaçon de marques (risque de confusion et facteurs DuPont)](https://www.uspto.gov/page/about-trademark-infringement) · Nolo — [Risque de confusion, expliqué](https://www.nolo.com/legal-encyclopedia/likelihood-confusion-how-do-you-determine-trademark-infringing.html)
- Outils de recherche de marques — [USPTO](https://www.uspto.gov/trademarks/search) · [EUIPO](https://www.euipo.europa.eu/en/trade-marks/before-applying) · [Base de données mondiale des marques OMPI](https://branddb.wipo.int/) · [TMview (mondial)](https://www.tmdn.org/tmview/)
- Alter, A. L., & Oppenheimer, D. M. (2006). [Predicting short-term stock fluctuations by using processing fluency](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC1482615/) (texte intégral en accès libre). *PNAS*, 103(24).
...
- Laham, S. M., Koval, P., & Alter, A. L. (2012). [The name-pronunciation effect: Why people like Mr. Smith more than Mr. Colquhoun](https://ppw.kuleuven.be/okp/_pdf/Laham2012TNPEW.pdf) (PDF texte intégral). *Journal of Experimental Social Psychology*, 48(3). Résumé en langage clair par [NYU Stern](https://www.stern.nyu.edu/experience-stern/faculty-research/adam-alter-names-study).
- Yorkston, E., & Menon, G. (2004). [A Sound Idea: Phonetic Effects of Brand Names on Consumer Judgments](https://pages.stern.nyu.edu/~gmenon/A%20Sound%20Idea.pdf) (PDF texte intégral). *Journal of Consumer Research*, 31(1).
- Topolinski, S., Zürn, M., & Schneider, I. K. (2015). [What's in and what's out in branding? A novel articulation effect for brand names](https://pmc.ncbi.nlm.nih.gov/articles/PMC4429570/) (accès libre). *Frontiers in Psychology*, 6.
- Snopes — [Did the Chevrolet Nova Fail to Sell in Spanish-Speaking Countries?](https://www.snopes.com/fact-check/chevrolet-nova-name-spanish/) (démystification du mythe)
- Wikipedia — [Mitsubishi Pajero / Montero naming](https://en.wikipedia.org/wiki/Mitsubishi_Pajero) (un vrai changement de nom pour la localisation)
