...
Aucune de ces étapes n'est difficile à elle seule. Ensemble, elles représentent douze actions manuelles réparties entre l'interface d'un bureau d'enregistrement, un panneau DNS et votre calendrier — pour une décision que l'IA est censée vous avoir déjà aidé à prendre en une seule requête. L'étape 12 ne relève pas de la légende : [le résumé de Wikipédia sur le processus de transfert de domaine](https://en.wikipedia.org/wiki/Domain_name_registrar) décrit comment le titulaire obtient « le code d'authentification (code de transfert EPP) auprès de l'ancien bureau d'enregistrement et supprime tout verrouillage du domaine », avant une période durant laquelle, historiquement, « le domaine ne peut pas être transféré de nouveau pendant 60 jours » — verrouillage que l'ICANN a voté en mars 2025 de ramener à 30 jours, avec un déploiement sur les 18 mois suivants. Dans les deux cas, une personne doit connaître l'existence de la règle et agir manuellement.
...
Selon sa propre documentation d'API publiée, oui pour les éléments documentés : l'enregistrement est finalisé via l'API ou MCP sans page de paiement, les enregistrements DNS peuvent être lus et modifiés par programmation, et le renouvellement automatique est exposé comme une option qu'un agent peut activer le jour de l'enregistrement. Il manque encore dans la documentation publique une fonction de plafond de dépenses côté serveur — ce garde-fou réside actuellement dans le client MCP ou la couche de règles que vous configurez autour de celui-ci.
...
Si votre agent peut déjà rédiger le code et choisir le nom, il n'y a aucune raison que les neuf étapes suivantes — vérification, achat, configuration du DNS, renouvellement, transfert — vous obligent de nouveau à parcourir une interface en cliquant. [Namefi](https://namefi.io) expose la recherche de domaines, l'enregistrement, la gestion DNS et les commandes de renouvellement sous forme d'outils qu'un agent compatible MCP peut appeler directement, avec une authentification par clé API ou signature de portefeuille, afin que l'échelle ne s'arrête pas au choix d'un nom.
...
- Wikipédia — [Bureau d'enregistrement de noms de domaine](https://en.wikipedia.org/wiki/Domain_name_registrar) — processus de transfert standard, rôle du code d'autorisation EPP et politique de l'ICANN adoptée en 2025 pour ramener le verrouillage suivant un transfert de 60 à 30 jours.
