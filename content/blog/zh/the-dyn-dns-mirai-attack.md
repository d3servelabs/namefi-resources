...
每次你输入一个域名，你的电脑都需要先将其转换为数字 [IP 地址](/zh/glossary/ip-address/)，才能建立连接。这个转换工作，正是 DNS（[域名系统](/zh/glossary/dns/)）的职责。它是人类可读的名称与机器实际地址之间的查询层。

Dyn 是这一查询服务的主要托管服务商之一。当一个网站将 DNS 外包给 Dyn 时，Dyn 的[域名服务器](/zh/glossary/nameserver/)就成为回答"这个域名在哪里？"的权威来源。The Register 在攻击发生时直白地描述了这种依赖关系：通过向 Dyn 发动攻击，谷歌和各大 ISP 运营的公共 [DNS 解析器](/zh/glossary/dns-resolver/)[无法联系 Dyn 来查询用户所需的主机名，导致用户无法访问使用 Dyn 提供 DNS 服务的网站](https://www.theregister.com/2016/10/21/dns_devastation_as_dyn_dies_under_denialofservice_attack/#:~:text=unable%20to%20contact%20Dyn%20to%20lookup%20hostnames)。
...
[Namefi](https://namefi.io) 建立在这样一个理念之上：域名应该像[互联网原生资产](/zh/glossary/internet-native-asset/)一样运作——所有权在密码学上可验证、可移植，同时与 DNS 完全兼容。可验证的、由所有者掌控的[域名所有权](/zh/glossary/domain-ownership/)并不能阻止僵尸网络的攻击——但它推动世界走向一个互联网：在那里，名称的控制权是可证明的、可审计的，而不是静默地依赖于某一服务商最糟糕的那一天。Mirai-Dyn 攻击提醒我们：你"拥有"的域名，其韧性只取决于为其提供应答的那一层。韧性，始于让所有权与控制权成为你真正能够验证的东西。
