...
**TLD（[顶级域名](/zh/glossary/tld/)）** 是域名中**最后一个点之后**的部分。在 `namefi.io` 中，TLD 是 `.io`；在 `google.com` 中，TLD 是 `.com`；在 `wikipedia.org` 中，TLD 是 `.org`。
...
| **二级域名（[SLD](/zh/glossary/second-level-domain/)）** | `namefi` | 你选择并拥有的独特名称。 |
...
**[gTLD](/zh/glossary/gtld/)** 是经典的通用扩展名。最初的集合规模较小，但在全球范围内广为人知：
...
| 简短易记 | `.io`、[`.top`](/zh/tld/top/)、[`.sbs`](/zh/tld/sbs/)、[`.now`](/zh/tld/now/) |

新 gTLD 为互联网提供了更大的发展空间：当所有优质 `.com` 都被注册时，`.xyz`、`.site` 和 `.app` 等后缀开辟了全新的、令人印象深刻的命名空间。
...
- **ICANN** — 互联网名称与数字地址分配机构是协调全球命名空间、制定 gTLD 政策并认证注册商的非营利组织。成立于 1998 年，是域名领域最接近"裁判"角色的机构。
- **IANA** — 互联网号码分配机构（由 ICANN 管理）维护着权威的**根区**：包含所有有效 TLD 及其对应注册机构的主列表。
- **注册局（Registries）** — 每个 TLD 由一个*注册局*运营，该组织负责维护该后缀的中央数据库。例如，**Verisign** 运营 `.com` 和 `.net`，**公共利益注册机构（PIR）** 运营 `.org`。ccTLD 注册局通常是国家机构——例如，`.ae` 由阿联酋的 TDRA 管理。
...
| TLD | 类型 | 最知名的用途 |
|-----|------|-------------|
| `.com` | gTLD | 任何企业的默认选择——最受信任、最具价值 |
| `.org` | gTLD | 非营利组织、社区、开源项目 |
| `.net` | gTLD | 科技、网络、基础设施 |
| `.io` | ccTLD（重新定位） | 初创公司、开发者、SaaS |
| `.ai` | ccTLD（重新定位） | 人工智能与科技 |
| `.app` | 新 gTLD | 移动端和网页应用（强制 HTTPS） |
| `.dev` | 新 gTLD | 开发者和工程团队 |
| `.tech` | 新 gTLD | 科技品牌和产品 |
| `.xyz` | 新 gTLD | 现代、灵活、跨代际 |
| `.shop` | 新 gTLD | 电商和零售 |
| `.vip` | 新 gTLD | 高端、专属、会员制品牌 |
| `.sbs` | 新 gTLD | "并肩同行"——经济实惠、富有表现力的名称 |

想深入了解某个特定后缀？浏览完整的 [TLD 指南库](/en/tld/)，包括 `.cloud`、`.online`、`.store`、`.site`、`.club`、`.world` 等数十个更多选项。
...
1. **`.com` 是否可用？** 它仍然是信任度和转售价值的黄金标准。如果你想要的 `.com` 空闲且价格合理，通常是最稳妥的默认选择。参阅为何 `.com` 仍是黄金标准。
2. **TLD 与你的用途是否匹配？** 初创公司适合 `.io` 或 `.ai`；电商店铺适合 `.shop` 或 `.store`；开发者工具适合 `.dev`。合适的后缀能够*描述*你所做的事情。
3. **你是否针对特定国家或地区？** 像 `.ae` 这样的 ccTLD 能彰显本地存在感，有助于提升本地搜索可见性——但请先确认注册资格要求。
4. **域名是否易记且具有品牌价值？** 在现代 TLD（`.xyz`、`.app`）下注册一个简短的二级域名，往往胜过一个冗长别扭的 `.com`。
5. **续费费用如何？** 有些 TLD 首年优惠价格低廉，但续费价格较高。务必确认长期价格，而非只看促销价。
6. **是否有限制条件？** 赞助型 TLD（`.gov`、`.edu`）和部分 ccTLD 有注册资格要求。新 gTLD 如 `.app` 和 `.dev` 默认强制使用 HTTPS。
...
- 在 NFT 市场上挂单或将其用作 [DeFi](/zh/glossary/defi/) 中的抵押品

……那么你选择的 TLD 就至关重要。

**Namefi** 是首个在以太坊主网上代币化真实 ICANN 域名的平台——也是首个在 Base 上实现这一功能的平台——覆盖上述众多 TLD，包括 `.com`、`.xyz`、`.io` 等。你可以在一个产品中同时拥有一个真实的、浏览器可解析的域名*和*钱包原生所有权。
...
- TLD 由顶层的 **ICANN** 和 **IANA**、运营各个后缀的**注册局**，以及向你销售域名的**注册商** 共同治理。
- 选择 TLD 需要考量信任度、适配性、成本，以及——越来越重要的——能否作为[代币化域名](/zh/blog/what-are-tokenized-domains/)被**上链**。

准备好在你喜欢的 TLD 下注册或[代币化域名](/zh/glossary/tokenized-domain/)了吗？访问 [namefi.io](https://namefi.io) 开始体验。
