...
一句话即可概括 **TLD 的定义**：它是[域名](/en/blog/what-is-domain/)中最右边的标签。人们也将其称为**域名扩展名**或**域名后缀**，但技术上正确的术语是*顶级域名*。它位于互联网命名层级的最顶端——这也正是其名称的由来。

> **TLD 含义速览：** *Top-Level Domain（顶级域名）* — 网址末尾的后缀（`.com`、`.org`、`.io`、`.ai`、`.xyz`），标识了[域名系统（DNS）](/en/glossary/dns/)的最高层级。

如果你曾搜索过 **"que es un TLD"**（西班牙语）或 **"qu'est-ce qu'un TLD"**（法语），任何语言的答案都是一样的：TLD 是域名的结尾部分，由 [ICANN](/en/glossary/icann/) 监管下的全球注册机构体系负责管理。
...
关于域名结构的详细介绍，请参阅[什么是域名？](/en/blog/what-is-domain/)以及我们的[域名术语指南](/en/blog/domain-terminology-guide/)。
...
- [`.com`](/en/tld/com/) — *commercial（商业）*，整个互联网的默认选择
...
**ccTLD** 是与某个国家或地区绑定的两字母顶级域名，基于 ISO 3166 国家代码列表。示例包括 `.us`（美国）、`.uk`（英国）、`.de`（德国）、`.cn`（中国）、[`.ae`](/en/tld/ae/)（阿联酋）和 [`.ac`](/en/tld/ac/)（阿森松岛）。

有趣的是，许多 ccTLD 因为字母拼写上的意义而被广泛应用于本国以外：

- [`.ai`](/en/tld/ai/) 在技术上是安圭拉的 ccTLD，但已成为人工智能公司的*首选*扩展名。
- [`.io`](/en/tld/io/) 属于英属印度洋领地，却在科技和初创公司品牌中占据主导地位（对应"I/O"）。
...
| 类别 | 示例 |
|------|------|
| 科技与互联网 | [`.app`](/en/tld/app/)、[`.dev`](/en/tld/dev/)、[`.tech`](/en/tld/tech/)、[`.cloud`](/en/tld/cloud/)、[`.click`](/en/tld/click/) |
...
新 gTLD 为互联网提供了更大的发展空间：当所有优质 `.com` 都被注册时，[`.xyz`](/en/tld/xyz/)、[`.site`](/en/tld/site/) 和 [`.app`](/en/tld/app/) 等后缀开辟了全新的、令人印象深刻的命名空间。
...
你可能也见过区块链原生后缀，如 `.eth` 或 `.crypto`。这些*不是* ICANN TLD——它们存在于传统 DNS 根之外，只能通过特殊钱包或解析器访问。Namefi 同样收录了这些后缀（参见 [`.eth`](/en/tld/eth/)），但值得注意的是，它们属于不同的类别。我们在[代币化域名与 Web3 域名](/en/blog/tokenized-domain-vs-web3-domain/)中详细阐述了这一区别。
...
- **ICANN** — [互联网名称与数字地址分配机构](/en/glossary/icann/)是协调全球命名空间、制定 gTLD 政策并认证注册商的非营利组织。成立于 1998 年，是域名领域最接近"裁判"角色的机构。
- **IANA** — 互联网号码分配机构（由 ICANN 管理）维护着权威的**根区**：包含所有有效 TLD 及其对应注册机构的主列表。
- **注册局（Registries）** — 每个 TLD 由一个*注册局*运营，该组织负责维护该后缀的中央数据库。例如，**Verisign** 运营 `.com` 和 `.net`，**公共利益注册机构（PIR）** 运营 `.org`。ccTLD 注册局通常是国家机构——例如，[`.ae`](/en/tld/ae/) 由阿联酋的 TDRA 管理。
- **注册商（Registrars）** — [注册商](/en/glossary/registrar/)是你购买域名的零售商。经 ICANN 认证的注册商（如 Namefi、GoDaddy 和 Namecheap）向公众销售域名，并将注册信息提交给注册局。
...
| [`.io`](/en/tld/io/) | ccTLD（重新定位） | 初创公司、开发者、SaaS |
| [`.ai`](/en/tld/ai/) | ccTLD（重新定位） | 人工智能与科技 |
| [`.app`](/en/tld/app/) | 新 gTLD | 移动端和网页应用（强制 HTTPS） |
| [`.dev`](/en/tld/dev/) | 新 gTLD | 开发者和工程团队 |
| [`.tech`](/en/tld/tech/) | 新 gTLD | 科技品牌和产品 |
| [`.xyz`](/en/tld/xyz/) | 新 gTLD | 现代、灵活、跨代际 |
| [`.shop`](/en/tld/shop/) | 新 gTLD | 电商和零售 |
| [`.vip`](/en/tld/vip/) | 新 gTLD | 高端、专属、会员制品牌 |
| [`.sbs`](/en/tld/sbs/) | 新 gTLD | "并肩同行"——经济实惠、富有表现力的名称 |

想深入了解某个特定后缀？浏览完整的 [TLD 指南库](/en/tld/)，包括 [`.cloud`](/en/tld/cloud/)、[`.online`](/en/tld/online/)、[`.store`](/en/tld/store/)、[`.site`](/en/tld/site/)、[`.club`](/en/tld/club/)、[`.world`](/en/tld/world/) 等数十个更多选项。
...
1. **`.com` 是否可用？** 它仍然是信任度和转售价值的黄金标准。如果你想要的 `.com` 空闲且价格合理，通常是最稳妥的默认选择。参阅[为何 `.com` 仍是黄金标准](/en/tld/com/)。
2. **TLD 与你的用途是否匹配？** 初创公司适合 [`.io`](/en/tld/io/) 或 [`.ai`](/en/tld/ai/)；电商店铺适合 [`.shop`](/en/tld/shop/) 或 [`.store`](/en/tld/store/)；开发者工具适合 [`.dev`](/en/tld/dev/)。合适的后缀能够*描述*你所做的事情。
3. **你是否针对特定国家或地区？** 像 [`.ae`](/en/tld/ae/) 这样的 ccTLD 能彰显本地存在感，有助于提升本地搜索可见性——但请先确认注册资格要求。
4. **域名是否易记且具有品牌价值？** 在现代 TLD（[`.xyz`](/en/tld/xyz/)、[`.app`](/en/tld/app/)）下注册一个简短的二级域名，往往胜过一个冗长别扭的 `.com`。
5. **续费费用如何？** 有些 TLD 首年优惠价格低廉，但续费价格较高。务必确认长期价格，而非只看促销价。
6. **是否有限制条件？** 赞助型 TLD（`.gov`、`.edu`）和部分 ccTLD 有注册资格要求。新 gTLD 如 [`.app`](/en/tld/app/) 和 [`.dev`](/en/tld/dev/) 默认强制使用 HTTPS。
...
[代币化域名](/en/blog/what-are-tokenized-domains/)是一个真实的、经 ICANN 认可的域名，其所有权*同时*以代币（通常是[NFT](/en/glossary/nft/)）的形式存储在你的钱包中。DNS 层的运行方式与以前完全相同；你只是在其之上获得了第二层可编程的所有权。
...
**Namefi** 是首个在以太坊主网上代币化真实 ICANN 域名的平台——也是首个在 Base 上实现这一功能的平台——覆盖上述众多 TLD，包括 [`.com`](/en/tld/com/)、[`.xyz`](/en/tld/xyz/)、[`.io`](/en/tld/io/) 等。你可以在一个产品中同时拥有一个真实的、浏览器可解析的域名*和*钱包原生所有权。

> 想了解两个层次如何配合？阅读[什么是代币化域名？](/en/blog/what-are-tokenized-domains/)，或访问 [namefi.io](https://namefi.io) 注册或代币化你的域名。
...
- TLD 由顶层的 **ICANN** 和 **IANA**、运营各个后缀的**注册局**，以及向你销售域名的**[注册商](/en/glossary/registrar/)** 共同治理。
- 选择 TLD 需要考量信任度、适配性、成本，以及——越来越重要的——能否作为[代币化域名](/en/blog/what-are-tokenized-domains/)被**上链**。
