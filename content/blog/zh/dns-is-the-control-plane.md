...
每一个应用程序和网站都需要一种方式来“查找”连接地址。这个互联网的地址簿被称为 DNS（[域名系统](/zh/glossary/dns/)，Domain Name System）。10月20日，AWS 内部的一个命名问题导致部分计算机无法通过名称找到关键的 AWS 数据库服务。如果地址簿不能在关键时刻提供正确的条目，即使是健康的系统也无法相互通信。
...
- **缓存，包括否定缓存 (Negative Caching)**。解析器会将答案存储一段时间，称为 TTL（[生存时间](/zh/glossary/ttl/)）。按照标准，它们也会缓存失败的结果。如果解析器在事故期间缓存了“未找到”的响应，它可能会一直提供该失败响应直到计时器过期，即使 AWS 已经修正了源头。（标准参考：[RFC 2308](https://datatracker.ietf.org/doc/html/rfc2308)，更新于 [RFC 9520](https://www.rfc-editor.org/rfc/rfc9520)）
...
- IETF / Cloudflare 文档 — DNS 否定缓存行为 (RFC 2308, RFC 9520) 以及用于多提供商权威部署的多签名者 [DNSSEC](/zh/glossary/dnssec/) 模式 (RFC 8901, 运营商文档)。（[RFC Editor](https://www.rfc-editor.org/rfc/rfc8901), [RFC Editor](https://www.rfc-editor.org/rfc/rfc9520)）