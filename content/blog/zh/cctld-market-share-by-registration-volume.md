...
有些 ccTLD 设有注册资格要求——您需要拥有本地地址、本地公司或国民身份证。`.jp` 和 `.no` 就完全属于这一类：JPRS 要求注册 `.jp` 需要拥有日本境内的永久邮政地址，而 Norid 则要求注册 `.no` 需具备挪威身份或组织资格，外加一个挪威通讯地址。`.fi` 则是一个有用的反例：Traficom 现在允许公司、组织和个人注册，且不受住所限制。受限区域在原始数量上永远无法与完全开放的区域抗衡，但其中存在的域名通常异常“干净”：滥用率低、停放率低、续费率高。如果您想寻找一个注册量真实可信的注册局，受限的 ccTLD 是一个很好的切入点。
...
在 Namefi，我们将上述许多 ccTLD（包括带有资格审查的受限 ccTLD）的注册路由到多个注册商后端。由于我们将所有权记录在链上进行代币化，而不是依赖单一注册商的账户级控制，后缀的选择就变成了一个路由决策，而非锁定决策。想在起步时使用 `.io`，随后为了本地市场将相同的品牌身份转移到 `.de`？这只是我们设计用来处理的标准转移工作流，而不是一个繁重的迁移项目。
