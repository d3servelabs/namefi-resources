...
如果您想详细了解[代币化域名](/zh/glossary/tokenized-domain/)，请从 [什么是代币化域名？](/zh/blog/what-are-tokenized-domains/) 开始阅读。
...
两者都涉及钱包中的 NFT。两者都被称为“域名”。在讨论中，两者都会在某种程度上牵扯到 ICANN——但方向截然相反。这两个类别的营销宣传往往模糊了它们之间的界限。
...
| [终端用户](/zh/glossary/end-user/)无需浏览器扩展 | 是 | 否 |
| 兼容 DNS 基础设施 | 是 | 不直接兼容 |
...
- 您希望该域名能在 [DeFi](/zh/glossary/defi/) 中用作链上抵押品，同时仍能作为正常网站运行。
...
- 您喜欢特定[顶级域名](/zh/glossary/tld/)（`.eth`、`.crypto`、`.x`）所附带的文化和社区属性。
...
您输入 `vitalik.eth`。一个具备 Web3 感知能力的客户端（如 MetaMask、DApp 或某些支持 [ENS](/zh/glossary/ens/) 的浏览器）查询以太坊上的 ENS [智能合约](/zh/glossary/smart-contract/)，获取关联的地址或内容哈希，并进行相应的渲染。不具备 Web3 感知能力的客户端（如没有扩展程序的 Chrome 浏览器、您的办公邮件服务器、您的 SSL CA）不知道 `.eth` 代表什么，也无法解析它。
