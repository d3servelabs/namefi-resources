...
您可能听说过“[代币化域名](/zh/glossary/tokenized-domain/)（tokenized domain）”、“将域名代币化（tokenize a domain）”或“域名代币化（domain tokenization）”等词，并想知道它们究竟意味着什么。代币化域名是一种新型域名吗？是一个仅存在于区块链上的名称吗？是 `.com` 的替代品吗？首先，将一个域名*代币化*到底意味着什么？
...
| 层面             | 它是什么                                            | 谁来维护                                  |
|------------------|-----------------------------------------------------|-------------------------------------------|
| DNS / 注册局     | [注册商](/zh/glossary/registrar/)和注册局的官方记录 | 经 ICANN 认证的注册商 |
| 链上代币         | 您[钱包](/zh/glossary/wallet/)中代表所有权的 NFT | 公有区块链上的[智能合约](/zh/glossary/smart-contract/) |
...
因为底层资产是真实的 DNS 域名，所以代币化域名仍然受制于续费规则、ICANN 政策、[UDRP](/zh/glossary/udrp/) 争议以及适用的法律。代币反映的是所有权；它并不能使域名免受现实世界规则的约束。
...
1. **注册** —— 通过受认证的注册商注册（或转入）一个真实的 DNS 域名。
2. **铸造** —— 作为域名代币化的一部分，一个代表该域名的 NFT 会被铸造到您的钱包中。
...
**方法：** 将真实的 ICANN 域名（`.com`、`.xyz`、`.io`、`.art` 及其他许多域名）代币化为 NFT，同时保持 DNS 层的完全可用。通过受认证的注册商使两层保持同步。

**Namefi 的与众不同之处：** Namefi 是**第一个在以太坊主网上将真实的 ICANN 域名代币化的平台，也是第一个在 Base 网络上实现这一壮举的平台**。由于 Namefi 代币化的域名存在于以太坊和 Base 上，得益于以太坊深厚成熟的 DeFi 生态系统，它们能自然地与**大多数主要的 NFT 市场和[借贷协议](/zh/glossary/lending-protocol/)**（如 OpenSea、Blur、NFTfi 等）集成。其他平台在区块链的选择上也做出了符合其自身目标的深思熟虑的决定；而就目前而言，以太坊和 Base 恰好能为 Namefi 用户提供与现有 NFT 和 DeFi 工具之间最广泛的开箱即用兼容性。
...
一些现有的 ICANN 注册商和注册局（例如 [GoDaddy](https://www.godaddy.com)、[Identity Digital](https://www.identity.digital)）已经宣布了试探性的代币化举措或合作伙伴关系。其覆盖范围和可用性差异很大，且它们的大部分核心业务仍然是传统的仅限 DNS 的注册。
...
- **设计上属于不同的命名空间。** Web3 域名（`.eth`、`.crypto`、`.x`、`.nft` 以及用户创建的 TLD）有意独立于 ICANN 根域之外，这使得它们能够快速迭代并尝试新的命名模式。代价是它们与传统的 DNS 层级结构并行存在，而非置于其中。
