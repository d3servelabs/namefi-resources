...
不是通过入侵服务器，也不是猜到了密码。他们填写了一份转移申请表，用盗刷的信用卡付款，然后静待一条刚刚出台的 ICANN 规则发挥作用。数小时之内，panix.com 的所有权被转移到一家澳大利亚公司，其 DNS 指向英国某服务商，电子邮件则被改道至加拿大——而真正运营 Panix 的人们，在毫无预警的情况下，就这样度过了周六夜晚。
...
而客户们无能为力。问题不在 Panix 的服务器上，那些服务器运行正常。问题在于域名系统的全球路由表——澳大利亚的一家注册商根据欺诈性请求告诉了全世界：panix.com 现在属于另一个人。
...
- **授权码（EPP 转移码）。** 现代通用顶级域名（gTLD）转移要求提供一个秘密授权码，该码由*转出方*注册商仅向经核实的注册人释放，从而使接收方注册商无法仅凭文书将域名转走。
...
[Namefi](https://namefi.io) 从相反的前提出发：对域名的控制应当是可证明的，而非假定的。通过将域名所有权表示为与 DNS 兼容的代币化链上资产，"谁持有这个名字"的问题变得可以通过密码学手段验证和审计——这是一条无法被接受了错误文书的注册商悄悄覆写的记录。转移只有在持有人的密钥授权时才会发生，而不是在一个无人值守的五日计时器到期时完成。默认状态是*拒绝*，而同意必须经过证明，而非仅仅是未被反对。
