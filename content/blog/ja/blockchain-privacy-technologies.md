...
![証明者が書類を背後に隠したまま、光る有効証明バッジを検証者に渡しており、根拠となる命題を明かさずに相手を納得させるゼロ知識証明の仕組みを表している](../../assets/blockchain-privacy-technologies-01-zero-knowledge.jpg)

[ゼロ知識証明](/ja/glossary/zero-knowledge-proof/)（ZKP）を使うと、一方の当事者である*証明者*は、ある命題が真であることを、それ以外の情報を明かさずに、もう一方の当事者である*検証者*に納得させられます。Ethereumの開発者向けドキュメントは、これを「ゼロ知識証明とは、命題そのものを明らかにせず、その命題の妥当性を証明する方法」であり、「『証明者』は主張を証明しようとする側、『検証者』はその主張の検証を担う側」であると端的に説明しています（[ethereum.org](https://ethereum.org/en/zero-knowledge-proofs/#:~:text=A%20zero%2Dknowledge%20proof%20is,without%20revealing%20the%20statement%20itself)）。

本物のゼロ知識プロトコルと認められる証明システムは、3つの性質を満たす必要があります。完全性（「入力が有効なら、ゼロ知識プロトコルは常に『真』を返す」）、健全性（「入力が無効なら、ゼロ知識プロトコルを欺いて『真』を返させることは理論上不可能」）、そしてゼロ知識性、すなわち「検証者は、命題が有効か無効かということ以外、その命題について何も知ることがない」です（[ethereum.org](https://ethereum.org/en/zero-knowledge-proofs/)）。具体的には、証明は証人（証明者だけが知る秘密）、チャレンジ（検証者が提示する問い）、レスポンスから構成されます。検証者はレスポンスを使って、証人そのものを見ることなく、証明者がその知識を持つことを確認できます。

**隠すもの：** 根拠となるデータまたは計算です。明らかになるのは、主張が真であることの証明だけです。
...
**トレードオフ：** ZK証明の生成は計算負荷が高く、証明回路はバッチ内のすべてのトランザクションを走査して検査を再実行します。そのため、オンチェーンでの検証は安価かつ高速でも、証明時間とハードウェアコストが現実的な制約になります。システムへの信頼は、数学と、一部の証明システムでは一度限りのトラステッドセットアップ・セレモニーを信頼することに集約されます。
...
**トレードオフ：** FHEはこの一覧で最も強力なプライバシー保証を提供します。計算中でさえ何も復号されないからです。一方で、平文での実行と比べて計算コストは群を抜いて高くなります。そのため、現在のFHEベースチェーンは、すべてのトランザクションではなく機密性が重要なロジックにFHEを使っており、FHEのハードウェア高速化をめぐる研究競争も活発です。
...
**信頼の前提：** セキュリティは、方式が破綻するまでに何人の参加者が不正になり得るかによって決まります。古典的な秘密分散方式は、能動的に悪意のある当事者が3分の1未満、または単に情報を得ようとする当事者が半数未満である限り、情報理論的セキュリティを提供します（[Wikipedia](https://en.wikipedia.org/wiki/Secure_multi-party_computation)）。つまりMPCは、「1つのカストディアンを信頼する」を「N人の当事者のうち、結託する者が多すぎないと信頼する」に置き換えます。
...
**トレードオフ：** MPCは、単一の秘密鍵を1台のデバイスに置くことによる単一障害点を避けられますが、当事者間の通信ラウンドによる遅延を加え、慎重なプロトコル設計を必要とします。MPC方式のセキュリティ保証は、前提とする誠実な多数派の閾値を超えるものではありません。この閾値は、数学的な前提であるだけでなく、社会的かつ運用上の前提でもあります。
...
**信頼の前提：** 純粋に数学に依存するZKP、FHE、MPCとは異なり、TEEではチップメーカーのハードウェアとファームウェアを信頼する必要があります。その信頼は試されてきました。SGXは「サイドチャネル攻撃を防げず」、研究者は実用的な突破を繰り返し実証しています。2017年には「同一システム上で動作するSGXエンクレーブから5分以内にRSA鍵」を抽出し、2018年のForeshadow攻撃では「投機的実行とバッファオーバーフローを組み合わせてSGXを回避」しました。その後もPlundervolt、LVI、SGAxe、ÆPIC Leakなどの脆弱性が見つかっています（[Wikipedia](https://en.wikipedia.org/wiki/Software_Guard_Extensions#:~:text=While%20this%20can%20mitigate%20many%20kinds%20of%20attacks%2C%20it%20does%20not%20protect%20against%20side%2Dchannel%20attacks)）。この歴史から、TEEは暗号学的に盤石な保証ではなく、実用的で高速な中間解として説明されるのが一般的です。
...
**トレードオフ：** TEEはネイティブに近い速度で動作し、FHEや負荷の高いZK証明よりはるかに高速です。このため遅延の影響を受けやすい用途に適しています。ただし、その速度は、サイドチャネル攻撃を受けた実例が記録されているハードウェアへの信頼によって得られます。そのため、最悪の場合の信頼の前提という点で、TEEベースのシステムは一般に純粋な暗号学的手法より弱くなります。
...
|---|---|---|---|---|---|
| ゼロ知識証明 | 根拠となるデータ／計算。明らかになるのは証明の妥当性のみ | 暗号学的な数学（一部のシステムではトラステッドセットアップも必要） | 証明生成は高コスト、検証は低コスト | 大規模な実運用段階（ロールアップ、シールド決済） | zkSync、Starknet、Zcash |
| 完全準同型暗号 | 計算を通じたすべてのデータ。計算事業者からも隠す | 暗号学的な数学（格子ベース） | 非常に大きな計算オーバーヘッド | 実運用初期。ハードウェア高速化の研究が進行中 | Zama、Fhenix |
| セキュアマルチパーティ計算 | 各当事者の個別の入力 | 参加者間の誠実な多数派／閾値 | 中程度。通信ラウンドが増加 | 成熟し、カストディで広く導入済み | Fireblocksなどの閾値署名カストディ事業者 |
| 信頼実行環境 | OSを含む、ほかのすべてのプロセスからデータ／コードを隠す | ハードウェア／ファームウェア事業者（チップメーカー） | ネイティブに近い速度 | 実運用段階だが、サイドチャネル攻撃の履歴が記録されている | Intel SGX、Oasis Sapphire |
