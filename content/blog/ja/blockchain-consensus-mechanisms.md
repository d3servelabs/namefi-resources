...
コンセンサスメカニズムは、あらゆる[トークン化ドメイン](/ja/blog/what-are-tokenized-domains/)を支える目に見えない基盤です。`.com`、`.ai`、`.io`のドメインが[NFT（非代替性トークン）](/ja/glossary/nft/)としてミントされると、チェーンのコンセンサスによって、オンチェーンの所有権記録と、そこに記録されるトークンの移転や売買の決済が保護されます。ただし、コンセンサスは、基となるDNSドメインの更新や登録の維持を担うレジストラおよびレジストリの手続きに取って代わるものではありません。[イーサリアム](/ja/glossary/ethereum/)上でミントされたドメインNFTは、EthereumのPoSが提供するチェックポイント型のファイナリティ保証を受け継ぎます。同じ資産がPoWチェーン上にあれば、そのチェーンの確率的な承認モデルを受け継ぎます。トランザクション手数料や、ユーザーが実用上の決済完了とみなせるまでの時間は、実行処理能力、ネットワーク需要、L1とL2のどちらを使うかにも左右され、PoWかPoSかだけで決まるものではありません。チェーンの基盤となるメカニズムと、それが実際に何を保護するのか、さらにそのシビル耐性とファイナリティの保証が何を意味するのかを理解することは、トークン化ドメインを含むあらゆるオンチェーン資産を評価するうえで重要です。
