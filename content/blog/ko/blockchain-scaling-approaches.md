...
[토큰화 도메인](/ko/glossary/tokenized-domain/)에서는 모든 민팅, 이전, DNS 업데이트, 담보 작업이 온체인 거래이므로 확장 방식이 중요합니다. 거래 비용과 최종성에 걸리는 시간이 어디에서 정산되는지에 따라 달라지기 때문입니다. 옵티미스틱 롤업에서 확인된 토큰화 `.com` 이전은 L2에서 저렴하고 빠르게 느껴질 수 있지만, 해당 롤업 거래는 [롤업 블록이 Ethereum에서 승인된 뒤에야 최종 확정됩니다](https://ethereum.org/en/developers/docs/scaling/optimistic-rollups/#:~:text=transactions%20conducted%20on%20the%20rollup%20are%20only%20final%20after%20the%20rollup%20block%20is%20accepted%20on%20Ethereum). 빠른 출금 브리지는 롤업 상태가 L1에서 더 빨리 최종 확정되도록 하지 않습니다. 출금 시 유동성 공급자가 대신 [보류 중인 L2 출금의 소유권을 넘겨받고 L1에서 사용자에게 먼저 지급합니다](https://ethereum.org/en/developers/docs/scaling/optimistic-rollups/#:~:text=A%20liquidity%20provider%20assumes%20ownership%20of%20a%20pending%20L2%20withdrawal%20and%20pays%20the%20user%20on%20L1). 일반적으로 수수료를 받으며, 정식 출금은 여전히 이의 제기 기간이 끝날 때까지 기다립니다. ZK 롤업의 같은 이전은 유효성 증명이 기록되는 즉시 L1을 기준으로 최종 확정됩니다. 사이드체인은 더 저렴할 수 있지만, 사이드체인에만 존재하는 도메인 NFT는 Ethereum이 아니라 해당 사이드체인의 더 작은 검증자 집합이 제공하는 보안을 물려받습니다. 이러한 트레이드오프를 이해하는 것은 도메인이 온체인에서 표현될 때 실제로 무엇을 소유하는지 이해하는 과정의 일부이며, [Web3 기초](/ko/topics/web3-foundations/) 전반에서도 중요한 실사 습관입니다.
