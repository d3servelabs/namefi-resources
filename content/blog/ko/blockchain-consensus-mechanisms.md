...
Solana가 PoS와 함께 사용하는 **역사 증명(PoH)**은 독립된 합의 메커니즘이 아니라 암호학적 시계입니다. “이전에 생성된 상태의 데이터를 해싱한 값을” 반복해서 이어 붙여 검증 가능한 타임스탬프를 체인에 직접 삽입합니다. 이를 통해 검증자들이 시간에 관해 서로 통신하지 않아도 사건 사이에 얼마나 많은 시간이 흘렀는지 증명하는 순서를 만듭니다([Solana: 역사 증명](https://solana.com/news/proof-of-history#:~:text=inserting%20data%20into%20the%20sequence%20by%20appending%20the%20hash%20of%20the%20data%20of%20the%20previously%20generated%20states)). 시간 자체를 검증 가능하게 만들면 검증자들이 거래를 병렬로 처리하고 검증할 수 있어 일반적으로 합의를 늦추는 통신 오버헤드가 줄어듭니다.

**권위 증명(PoA)**은 경제적·연산 비용을 완전히 없애고 평판을 사용합니다. “잘 알려진 기업”이나 검증된 개인처럼 신원이 공개되고 이미 알려진 고정 검증자 집합이 블록 생성을 맡습니다. 지분 슬래싱이나 낭비되는 연산 대신 평판과 법적 책임으로 부정행위를 억제합니다([ethereum.org: 권위 증명](https://ethereum.org/en/developers/docs/consensus-mechanisms/poa/#:~:text=The%20reputation%20in%20this%20context%20is%20not%20a%20quantified%20thing)). 탈중앙화를 포기하는 대신 속도와 낮은 비용을 얻기 때문에 공개된 적대적 네트워크보다는 주로 프라이빗 체인, 테스트넷, 로컬 개발 네트워크에서 사용됩니다.
