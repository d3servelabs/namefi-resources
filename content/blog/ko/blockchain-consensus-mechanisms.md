...
합의 메커니즘은 모든 [토큰화 도메인](/ko/blog/what-are-tokenized-domains/)을 떠받치는 보이지 않는 기반입니다. `.com`, `.ai`, `.io` 도메인이 [NFT](/ko/glossary/nft/)로 발행되면 체인의 합의는 온체인 소유권 기록과 체인에 기록된 토큰 이전 또는 매매 결제를 보호합니다. 즉 합의가 확인하고 순서를 정하는 범위는 해당 체인 원장에 기록된 상태입니다. 합의가 기초 DNS 도메인을 갱신하거나 등록의 유효성을 유지하는 등록대행자 및 레지스트리 절차를 대신하지는 않습니다. [Ethereum](/ko/glossary/ethereum/)에서 발행된 도메인 NFT는 Ethereum PoS의 체크포인트 기반 최종성 보장을 이어받고, 같은 자산이 PoW 체인에 있다면 해당 체인의 확률적 확인 모델을 따릅니다. 거래 수수료와 사용자가 체감하는 실질적인 결제 완료 시간은 실행 용량, 네트워크 수요, L1 또는 L2 사용 여부에도 좌우되며, PoW와 PoS 중 무엇을 쓰느냐만으로 결정되지 않습니다. 어떤 메커니즘이 체인의 기반인지, 그 메커니즘이 실제로 무엇을 보호하는지, 그리고 시빌 저항성과 최종성 보장이 무엇을 의미하는지 이해하는 일은 토큰화 도메인을 포함한 모든 온체인 자산을 평가하는 데 필요합니다.
