...
共识机制是每个[代币化域名](/zh-CN/blog/what-are-tokenized-domains/)之下看不见的基础。当 `.com`、`.ai` 或 `.io` 域名被铸造成 [NFT（非同质化代币）](/zh-CN/glossary/nft/) 时，谁拥有该代币的记录——以及之后的每一次转移、出售或续费——其可信度完全取决于保障该资产所在链的共识机制。铸造在[以太坊](/zh-CN/glossary/ethereum/)上的域名 NFT 会继承 PoS 的快速、低成本终局性，以及其数以十万计的验证者集合；同一资产若位于 PoW 链上，则会继承概率性终局性和高得多的交易成本。了解一条链采用哪种底层机制，以及其抗女巫攻击和终局性保障实际意味着什么，是评估任何链上资产（包括代币化域名）的一部分。
