...
每一步单独看都不难。但合起来，这就是分散在注册商后台、DNS 面板和日历中的 12 项人工操作；而 AI 原本只用一个提示词就帮您做出了决定。第 12 步并非坊间传闻：[Wikipedia 对域名转移流程的概述](https://en.wikipedia.org/wiki/Domain_name_registrar)说明，注册人需要从原注册商取得“认证码（EPP 转移码）”并“移除所有域名锁定”；历史上其后还有一个“域名在 60 天内不能再次转移”的窗口。ICANN 于 2025 年 3 月投票决定将该锁定期缩短为 30 天，并在随后 18 个月内逐步实施。无论是哪种情况，人都必须知道这项规则存在并手动采取行动。
...
根据其已发布的 API 参考资料，对于已记录的部分，答案是肯定的：注册可以通过 API 或 MCP 完成，无需结账页；DNS 记录可以通过程序读取和写入；自动续期可作为智能体在注册日设置的开关。尚未公开记录的是服务端的支出上限原语——这项护栏目前位于您围绕 MCP 客户端设置的策略层中。
...
如果您的智能体已经能起草代码并挑选名称，就没有理由让接下来的九个步骤——查询、购买、配置 DNS、续期、转移——又退回到您在后台逐项点击。[Namefi](https://namefi.io) 将域名搜索、注册、DNS 管理和续期控制作为 MCP 兼容智能体可直接调用的工具提供，并使用 API 密钥或钱包签名进行认证；因此，这个阶梯无需止步于名称。
...
- Wikipedia — [域名注册商](https://en.wikipedia.org/wiki/Domain_name_registrar) ——说明标准转移流程、EPP 授权码的作用，以及 ICANN 于 2025 年将转移后锁定期从 60 天缩短至 30 天的政策。
