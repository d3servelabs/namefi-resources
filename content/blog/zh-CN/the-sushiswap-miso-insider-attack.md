...
2021 年 9 月，运营 SushiSwap MISO 发射台的团队既没有遭到网络钓鱼，也没有丢失[私钥](/zh-CN/glossary/private-key/)，更没有部署存在漏洞的[智能合约](/zh-CN/glossary/smart-contract/)。他们做的事情平常得多：信任了一位贡献者。一名拥有代码提交权限的匿名承包商将自己的[钱包](/zh-CN/glossary/wallet/)地址植入[拍卖](/zh-CN/glossary/auction/)前端，推送后让部署流水线完成了剩余的工作。当一场 NFT 拍卖结算时，大约 **864.8 ETH——约 300 万美元**——非但没有流向组织此次销售的项目方，反而流进了那位悄悄改写资金去向的开发者口袋。
...
像 MISO（Minimal Initial SushiSwap Offering，最小化初始 SushiSwap 发行）这样的 [DeFi](/zh-CN/glossary/defi/) 发射台，存在的目的只有一个：将来自陌生人群体的资金，路由到发起代币或 NFT 销售的项目方。为此，它将[链上](/zh-CN/glossary/on-chain/)经过审计的智能合约与链下的 Web 前端缝合在一起。用户与前端交互，前端告诉他们的钱包要签署什么交易。
...
MISO 之所以蒙受损失，是因为*价值的去向*可以被系统所信任的人悄悄改写，而没有任何人在上线前核实这一变更。这种失效模式并非 DeFi 发射台所独有。它与域名的处境如出一辙——域名的所有权或 DNS 记录，可能被任何恰好持有相关访问权限的人悄悄更改：[注册商](/zh-CN/glossary/registrar/)账户、内部管理面板、持有凭证的承包商。
...
[Namefi](https://namefi.io) 的应对方式，是将[域名所有权](/zh-CN/glossary/domain-ownership/)视为可验证、防篡改的资产，而非某个私人账户中的条目。代币化所有权使控制权可在链上被审计和转移，同时保持与 DNS 的兼容——"谁拥有这个，谁被允许修改它"由此成为一个可以核验的事实，而非需要盲目延伸的信任。MISO 那名承包商之所以能改写收款地址，恰恰是因为系统没有一个强制执行的、可独立检查的"此变更是否已授权？"的答案。Namefi 从供应链攻击中汲取的教训是：所有权和控制权应在设计上就是可证明的，从而让*受信任*与*已核验*之间的危险缺口永远不会出现。
