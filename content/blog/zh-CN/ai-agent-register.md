...
如果你已确定使用 Claude，且希望阅读附有真实对话记录的完整注释式教程，[使用 Claude 购买域名：Namefi MCP 分步指南](/zh-CN/blog/claude-mcp-domains/)会比本文压缩版的 Claude 章节更深入。本文是中心页；那篇文章以及文中其他链接则是向外延伸的分支。
...
<!-- TODO：确认——Claude Desktop 的自定义连接器界面为 `x-api-key` 这类请求头提供的确切字段 --> 如果你的 Desktop 配置中没有明显可粘贴密钥的位置，当前已核验可用于写入操作的路径是 Claude Code（下文）；而无需密钥的只读工具，例如可用性搜索，则可通过连接器使用。包括连接后界面流程在内的完整教程请见[使用 Claude 购买域名：Namefi MCP 分步指南](/zh-CN/blog/claude-mcp-domains/)。
...
`${env:NAMEFI_API_KEY}` 会在连接时解析为该环境变量保存的值。有关同一配置的精简版本，请参阅[Namefi MCP 快速入门：Claude Code、Cursor 与 Windsurf](/zh-CN/blog/mcp-quickstart/)。
...
llms.txt 是一种纯文本约定：站点会在根路径发布机器可读的索引，让 AI 智能体无需抓取渲染后的文档页面也能发现 API 的用途。如果你想看完整版本而非压缩摘要，可直接在 [namefi.io/llms.txt](https://namefi.io/llms.txt) 阅读 Namefi 的文件；关于这一约定本身，参阅[llms.txt 与域名：任何 AI 智能体都能读取的 API](/zh-CN/blog/llms-txt/)。
...
[什么是智能体原生域名注册商？](/zh-CN/blog/agent-native/)为评估任一注册商的面向智能体能力（包括 Namefi）提供了更完整的检查清单：可发现性、机器可读的错误信息，以及不假定人类持有信用卡的付款路径。
...
可以。钱包签名的 x402 结账路径不需要 Namefi 账户或 API 密钥，只需要一个已充值的钱包。上文付款章节概述了该流程；完整教程请见[使用加密钱包支付域名：无需账户](/zh-CN/blog/wallet-checkout/)。
...
**[生成 Namefi API 密钥](https://namefi.io/api-key)**，或进一步阅读[包含完整对话记录的 Claude 教程](/zh-CN/blog/claude-mcp-domains/)和[智能体原生注册商的横向比较](/zh-CN/blog/cf-namecom-namefi/)。如需了解本指南底层的组成部分，请参阅[Namefi MCP 服务器：面向 AI 智能体的域名工具](/zh-CN/blog/namefi-mcp/)、[Namefi MCP 快速入门：Claude Code、Cursor 与 Windsurf](/zh-CN/blog/mcp-quickstart/)、[使用加密钱包支付域名：无需账户](/zh-CN/blog/wallet-checkout/)以及[llms.txt 与域名：任何 AI 智能体都能读取的 API](/zh-CN/blog/llms-txt/)。
