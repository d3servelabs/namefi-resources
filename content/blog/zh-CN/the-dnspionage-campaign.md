...
没有 Web 服务器遭到入侵，没有主页被植入恶意软件，没有篡改页面，没有勒索信息，应用日志里也没有任何明显的蛛丝马迹。攻击者根本无需破门而入。他们穿过了几乎无人把守的那扇门：**[DNS](/zh-CN/glossary/dns/) 记录**——那条告诉全世界某个域名的邮件和网站究竟在哪里的记录。他们悄悄地、凭借有效凭据、躲在有效 TLS 证书的掩护下修改了这条记录——世界各地的流量便毫无怨言地按新指令奔涌而去。
...
每当你向某个政府部门发送邮件、登录企业 VPN，或打开一个网页邮箱，你的设备首先会向 DNS 提问：*这个名字对应哪个 [IP 地址](/zh-CN/glossary/ip-address/)？* 无论 DNS 如何回答，你都会信任它。你的邮件客户端连向那里，VPN 在那里完成认证，浏览器在那里交出会话凭据。DNS 是整个互联网的地址簿，而几乎没有任何机制会核实这本地址簿是否被人修改过。
...
**第一步：拿到地址簿的钥匙。** 攻击者没有破解 DNS 密码学，他们直接登录了系统。FireEye 描述了两条路径："[一种方法是使用窃取的凭据登录 DNS 提供商的管理界面并修改 DNS A 记录，以拦截电子邮件流量。另一种方法是入侵受害者的域名注册商账户后修改 DNS NS 记录](https://www.securityweek.com/iran-linked-dns-hijacking-attacks-target-organizations-worldwide/#:~:text=One%20method%20involves%20logging%20into%20a%20DNS%20provider)。"被盗的[注册商](/zh-CN/glossary/registrar/)和 DNS 托管商凭据就是万能主密钥。掌握注册商登录凭据，就掌握了域名——而域名掌控着指向它的一切。
...
第四项是关键所在。CISA 不仅要求各机构锁好门，还要求他们盯着公开的证书账本，查看是否有人已经用一把复制的钥匙开过门。DNSpionage 将证书透明度从一项小众的 PKI 特性变成了应对国家级 [DNS 劫持](/zh-CN/glossary/dns-hijacking/)的前线检测工具。
