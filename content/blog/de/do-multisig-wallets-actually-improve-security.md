...
Fast jede Multisig-Signatur erfolgt über eine Schnittstelle – die Web-UI von Safe, ein Wallet-Plugin, ein benutzerdefiniertes Dashboard. Wenn diese Schnittstelle kompromittiert ist ([DNS-Hijacking](/de/glossary/dns-hijacking/), Supply-Chain-Angriff auf eine Abhängigkeit, bösartige Browser-Erweiterung), kann der Angreifer Unterzeichner A "1 ETH an alice.eth senden" anzeigen, während er tatsächlich "1000 ETH an attacker.eth senden" zur Signatur an die Hardware-Wallet übermittelt.
...
In allen drei Fällen lautet die Antwort nicht "Nutze Single-Key-Verwahrung". Sie lautet "Nutze Multisig *richtig* oder nutze einen Verwahrer, der dies tut." Aber so zu tun, als ob allein die Art des Contracts [Sicherheit](/de/glossary/collateral/) bietet, unabhängig von der betrieblichen Praxis, ist der Grund, warum aufsehenerregende Verluste passieren.
