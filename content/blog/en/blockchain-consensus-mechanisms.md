...
**Proof of History (PoH)**, used by Solana alongside PoS, is not a standalone consensus mechanism but a cryptographic clock. It inserts verifiable timestamps directly into the chain by repeatedly hashing "the data of the previously generated states," creating a sequence that proves how much time passed between events without validators needing to communicate about time ([Solana: Proof of History](https://solana.com/news/proof-of-history#:~:text=inserting%20data%20into%20the%20sequence%20by%20appending%20the%20hash%20of%20the%20data%20of%20the%20previously%20generated%20states)). That clock gives validators a verifiable ordering for consensus, but it is not what executes transactions in parallel. Parallel execution comes from **Sealevel**: Solana transactions declare every account they will read or write, letting the runtime execute non-overlapping transactions — and transactions that only read the same state — concurrently ([Solana: Sealevel](https://solana.com/news/sealevel---parallel-processing-thousands-of-smart-contracts#:~:text=The%20reason%20why%20Solana%20is%20able%20to%20process%20transactions%20in%20parallel,transactions%20that%20are%20only%20reading%20the%20same%20state%20to%20execute%20concurrently%20as%20well)).

**Proof of Authority (PoA)** replaces open mining or stake-based validation with a permissioned set of authorized signers. Relative to PoW, this sharply reduces the resource cost of producing blocks; ethereum.org describes PoA as avoiding PoW's demand for high-resource mining ([ethereum.org: Proof-of-authority](https://ethereum.org/en/developers/docs/consensus-mechanisms/poa/#:~:text=as%20it%20overcomes%20the%20need%20for%20high%20quality%20resources%20as%20PoW%20does)). It does not eliminate network operation or security costs. The security and governance burden shifts to trusted validator identities, reputations, and signer-admission rules: PoA requires trust in known signers, often established through KYC or recognizable organizations ([ethereum.org: trusted signers](https://ethereum.org/en/developers/docs/consensus-mechanisms/poa/#:~:text=Proof%2Dof%2Dauthority%20requires%20trusting%20a%20set%20of%20authorized%20signers,if%20a%20validator%20does%20anything%20wrong%2C%20their%20identity%20is%20known)), and the implementation ethereum.org describes lets signers vote to add or remove peers ([ethereum.org: signer admission](https://ethereum.org/en/developers/docs/consensus-mechanisms/poa/#:~:text=Each%20signer%20votes%20for%20the%20addition%20or%20removal%20of%20a%20signer%20in%20their%20block%20when%20they%20create%20a%20new%20block)). It trades decentralization for speed and low operating cost, which is why it's mostly used on private chains, testnets, and local development networks rather than public, adversarial ones.
...
Consensus mechanisms are the invisible foundation under every [tokenized domain](/en/blog/what-are-tokenized-domains/). When a `.com`, `.ai`, or `.io` domain is minted as an [NFT](/en/glossary/nft/), the chain's consensus secures the on-chain ownership record and any token transfers or sale settlement recorded there. It does not replace the registrar and registry processes that renew the underlying DNS domain or keep its registration valid. A domain NFT minted on [Ethereum](/en/glossary/ethereum/) inherits Ethereum PoS's checkpoint-based finality guarantees; the same asset on a PoW chain inherits that chain's probabilistic confirmation model. Transaction fees and the time users experience before practical settlement also depend on execution capacity, network demand, and whether they use L1 or L2 — they are not determined by PoW versus PoS alone. Understanding which mechanism underlies a chain, what it actually secures, and what its Sybil-resistance and finality guarantees mean is part of evaluating any on-chain asset, tokenized domains included.
...
- [Solana — Sealevel: Parallel Processing Thousands of Smart Contracts](https://solana.com/news/sealevel---parallel-processing-thousands-of-smart-contracts)
