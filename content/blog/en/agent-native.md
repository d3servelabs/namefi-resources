...
`llms.txt` is a plain-text file an agent reads once to learn what a site or API is and how to use it — the same role `robots.txt` plays for crawlers, but written for language models. [MCP](https://modelcontextprotocol.io) is a live protocol connection an agent's client opens to a server exposing callable tools. They're complementary: llms.txt is discovery, MCP is the connection an agent uses to act. See [llms.txt for Domains: An API Any AI Agent Can Read](/en/blog/llms-txt/) for more on the discovery half.
