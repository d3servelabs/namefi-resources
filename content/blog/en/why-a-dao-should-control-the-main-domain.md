...
Here is the part that is easy to miss until you have lived through it: in an online movement, the constitutional crisis does not get settled by argument. It gets settled by *control of the front door*.

The main domain — the `.com`, the `.org`, the `.eth` that people type from memory, that wallets resolve, that Google ranks first, that the "official" email comes from — is the center of the stage made concrete. Whoever controls it does not have to *win* the legitimacy argument. They get to *be* the legitimate one by default, because that is the address everyone already trusts. The fork can have the better claim, the bigger community, the original contributors — and still lose, because the audience keeps walking through the door it already knows.

This is why, in practice, the domain is the prize in nearly every movement-level schism. It is the one asset where soft questions of legitimacy collapse into a hard question of access control: who holds the login, who can change the nameservers, whose signature moves the asset. The constitution you actually run on is whoever can point the domain.
...
- **Verifiable.** Who can move the asset, and under what threshold, is visible on-chain. You do not have to trust that the right people hold the registrar password — you can read the rule.
...
Give the front door to the body that is supposed to speak for the whole movement. Make it explicit, make it verifiable, make it survive any one person. Put the main domain under the DAO before you need to.
