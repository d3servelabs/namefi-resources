...
- **`.gl` (Greenland).** Google's URL shortener, `goo.gl`, was built on Greenland's code. Google stopped accepting newly shortened URLs in 2018, but this was a product decision rather than a failure of the `.gl` registry. In an August 2025 update, Google said it would preserve actively used `goo.gl` links and deactivate only links that had already shown an inactivity warning. IANA continues to list TELE Greenland A/S as the `.gl` ccTLD manager.
...
## The catch: a ccTLD is a locally governed namespace
...
Here is the part most "10 clever domain hacks" listicles skip, and it's the part that separates a hobbyist from someone who can actually value one of these. **When you register a domain hack, you depend on the policies and operation of its ccTLD manager.** A `.com` operates under ICANN's gTLD contracts and consensus policies; a ccTLD's management is primarily a local matter, shaped by its manager in consultation with the local internet community and subject to applicable local law. Government involvement varies widely—from light participation to direct operation—so it is inaccurate to assume that every ccTLD is run by its national government.
...
3. **What's the registry's policy and stability?** Before you value a `.ly`, `.io`, or any ccTLD hack, learn whether the registry welcomes third-party registration, what content rules apply, and how the manager handles disputes and policy changes. The `vb.ly` case shows why those rules can have practical consequences. Diligence can identify current restrictions, although it cannot eliminate the risk of later policy changes. A beautiful hack on a restrictive or uncertain ccTLD may deserve a discount relative to a comparable gTLD.
4. **Is the extension already a proven hack market?** `.io`, `.co`, `.me`, `.gg`, and `.ly` have established demand, [liquidity](/en/glossary/domain-liquidity/), and a track record of brands paying for them. A novelty ccTLD with no buyers is a domain you'll hold forever. Liquidity is part of the price.
...
This is the gap [Namefi](https://namefi.io) is built to narrow: tokenized ownership can make the on-chain ownership record and token transfer easier to audit, and an atomic marketplace sale can reduce settlement risk. It does not automatically put registrar recovery, nameserver delegation, or an external DNS provider behind the same authorization boundary. A token transfer also does not by itself change DNS configuration, but continued resolution still depends on those separate systems remaining correctly configured and available. Clever is fun. An auditable transfer with each control plane secured is what lets you trade the asset underneath it.
