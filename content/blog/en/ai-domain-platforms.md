...
These are not competing standards; the strongest platforms layer all three, using llms.txt for discovery, an MCP server for the actual tool calls, and the REST API underneath both. For why that middle layer is worth building at all when the REST API already exists — runtime tool discovery, JSON Schema arguments, errors written for a model to recover from — see [Why MCP Exists When We Already Have REST APIs and CLIs](/en/blog/mcp-vs-rest-api/).
