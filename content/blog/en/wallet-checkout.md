...
Two details matter for safety on this manual path: **signatures expire after 300 seconds, and nonces are single-use.** Once 300 seconds have elapsed, or once a request using the nonce has been accepted, a captured signature cannot be replayed successfully. Namefi's documentation also specifies that the live EIP-712 type definitions must be fetched from its `/v-next/eip712/` endpoints at request time rather than hardcoded by an integration, since the exact schema a signature must match can change.
...
**What it does constrain.** Each path requires the wallet to sign for the current request rather than handing Namefi a standing credential. The replay controls differ by protocol: the manual EIP-712 path expires after 300 seconds and consumes a single-use nonce; x402 uses an EIP-3009 authorization tied to a specific amount and recipient, bounded by `validAfter`/`validBefore`, and protected by a nonce; the MPP client signs the server-issued challenge, so its expiry and replay controls are whatever that challenge specifies. The wallet never hands Namefi standing authority to initiate future charges on its own. Compare that to a stored card: once a merchant has your card token, nothing about the token itself limits what they charge next month, or whether a compromised system reuses it. A wallet's private key never leaves the wallet in any of these flows — the agent asks the wallet to produce a signature for one specific request, and that's the entire scope of what happens.

**What it does not constrain on its own.** Namefi's documentation does not describe a built-in, per-transaction dollar spend cap enforced by the protocol itself — the protocol-specific expiry and replay controls limit when and how an authorization can be reused, not how much a single signed request can authorize.<!-- TODO: confirm with team — whether Namefi's x402/MPP endpoint enforces any server-side maximum payment amount independent of what the client requests to sign --> In practice, the actual spending discipline for an agent comes from outside this mechanism: how much USDC you fund the wallet with, and whatever policy layer — a [multi-sig](/en/glossary/multi-sig/) wallet requiring a second approval, or a human confirmation step before the agent may sign at all — you put between the agent and the wallet's [private key](/en/glossary/private-key/). [What Is an Agent-Native Domain Registrar?](/en/blog/agent-native/) and [How to Register a Domain with Your AI Agent on Namefi](/en/blog/ai-agent-register/) cover the same point from the guardrails side: fund the wallet with only what you're comfortable an unattended process spending, and decide up front where a human needs to approve.

That combination — no standing credential, a bounded per-transaction authorization, and funding as the practical spend limit — makes this a genuinely different shape of risk than a card on file, not just a crypto-flavored version of the same thing. A leaked card number or a compromised billing token can be charged repeatedly until someone notices and cancels it. A captured payment authorization is rejected when its own protocol-specific expiry or replay condition is met: the manual EIP-712 path rejects it after 300 seconds or once its nonce is consumed; x402's EIP-3009 authorization rejects it outside `validAfter`/`validBefore` or after its nonce is used; and an MPP credential follows the expiry and replay conditions encoded by its signed challenge.
...
A captured signature may remain usable until its own expiry or replay control rejects it; the three paths do not share one universal rule. In the manual EIP-712 path, signatures expire after 300 seconds and each nonce can only be used once. The x402 flow's EIP-3009 authorization is valid only within its `validAfter`/`validBefore` window, and its nonce cannot be used twice. MPP uses a signed challenge, so its expiry and replay conditions must be checked in that challenge rather than assumed to match either of the other paths.
...
It constrains a different set of risks rather than eliminating risk altogether. There's no standing credential a compromised system can reuse indefinitely, and each payment requires a new request-level signature. The replay controls differ: manual EIP-712 uses a 300-second expiry and single-use nonce; x402's EIP-3009 authorization uses `validAfter`/`validBefore` and a nonce; MPP follows the conditions in its signed challenge. None of those controls caps how much a single signed request can authorize, so the practical ceiling on what an agent can spend still comes from how much you fund the wallet with and any additional approval policy (like a multi-sig) you put in front of it.
