...
Do not transfer first. Once a domain leaves your control, your leverage is gone, and clawing it back means a slow, uncertain dispute rather than a refund. This matters even more because of how transfers actually work. Moving a domain to another [registrar](/en/glossary/registrar/) requires you to hand over the authorization code — Wikipedia notes the buyer [obtains the authentication code (EPP transfer code) from the old registrar](https://en.wikipedia.org/wiki/Domain_name_transfer#:~:text=obtains%20the%20authentication%20code%20%28EPP%20transfer%20code%29) — and once that code is out and the transfer completes, the name is theirs. The same source notes [the process may take about five days](https://en.wikipedia.org/wiki/Domain_name_transfer#:~:text=The%20process%20may%20take%20about%20five%20days), and after it lands there's a lock period before it can move again, which used to be 60 days and, per Wikipedia, that [60-day transfer lock policy was eliminated, replaced with a 30-day lock period](https://en.wikipedia.org/wiki/Domain_name_transfer#:~:text=replaced%20with%20a%2030%2Dday%20lock%20period). None of that helps you if you've already given the name to a thief.
...
**Always use real, escrow-backed settlement, and pick it yourself.** Insist on a reputable escrow service or [marketplace](/en/glossary/marketplace/) that *you* propose. The neutrality only works when you chose the referee. If a buyer refuses your escrow and demands theirs, the deal is over — that refusal is the diagnosis. Our full walkthrough of how this should look is in [domain escrow explained](/en/blog/domain-escrow-explained/) and the seller's checklist in [how to sell a domain name you own](/en/blog/how-to-sell-a-domain-name-you-own/).
...
[Namefi](https://namefi.io) narrows that gap by tokenizing ownership of real ICANN domains, so control of the name and settlement of the deal can be verified and exchanged together, with DNS continuity so the name keeps resolving through the handover. When the asset is auditable and the transfer is atomic, the "who moves first" problem — the thing every scam in this guide attacks — has a lot less room to hide. We dig into that shift in [how tokenized marketplaces replace escrow](/en/blog/how-tokenized-marketplaces-replace-escrow/).
