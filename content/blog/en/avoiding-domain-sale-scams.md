---
title: "How to Avoid Domain Sale Scams"
date: '2026-06-21'
language: en
...
Almost every domain-sale scam is a variation on one move: getting you to give up either the domain or the money before the other side has actually delivered. Once you can see that move, the defenses are simple and they barely slow down a legitimate deal. This guide walks the scams you'll actually meet, then the habits that defeat all of them. It's part of the [domain flipping skills series](/en/blog/domain-flipping/), and it pairs closely with the legal side covered in [domain flipping and the law](/en/blog/domain-flipping-and-the-law/).

## The one trick behind every scam: who moves first

Every sale between strangers has the same standoff. The buyer doesn't want to pay before they get the domain, and the seller doesn't want to transfer the domain before they get paid. Somebody has to go first, and going first means trusting the other side. That's the whole problem [escrow](/en/glossary/escrow/) was invented to solve, and we cover the mechanics in [domain escrow explained](/en/blog/domain-escrow-explained/).
...
The red flag is not simply who named the escrow service; it is pressure to use a provider or link you have not independently verified. Check the provider's legal identity, licensing where applicable, official domain, and contact details through authoritative sources. California's Department of Financial Protection and Innovation advises users to [verify that an online escrow company is licensed and warns that a buyer or seller who insists on a particular service may be steering them to a fraudulent site](https://dfpi.ca.gov/regulated-industries/escrow-law/online-escrow-companies/10-tips-to-avoid-online-escrow-fraud/). Start the transaction by typing the provider's known official domain yourself or by signing in to an account you opened there — never through the counterparty's link — and verify the transaction inside that authenticated account. Escrow.com's own fraud guidance likewise says to [go directly to the company's website in your browser rather than clicking a link](https://www.escrow.com/what-is-escrow/fraud-prevention). A provider suggested by the buyer can be legitimate; the protection comes from independently confirming and using the authentic service.
...
The engine here is the chargeback. Wikipedia defines it as [a return of money to a payer of a transaction, especially a credit card transaction](https://en.wikipedia.org/wiki/Chargeback#:~:text=A%20chargeback%20is%20a%20return%20of%20money%20to%20a%20payer). Crucially, [the chargeback reverses a money transfer from the consumer's bank account, line of credit, or credit card](https://en.wikipedia.org/wiki/Chargeback#:~:text=The%20chargeback%20reverses%20a%20money%20transfer). That reversal can land days or weeks after the payment looked complete, and it's a standard buyer protection that scammers exploit by paying with a stolen or disputable instrument. By the time the chargeback hits, you've already transferred the domain and refunded the "overpayment." You lose both.
...
Do not transfer first. Once a domain leaves your control, your leverage is gone, and clawing it back means a slow, uncertain dispute rather than a refund. This matters even more because of how transfers actually work. Moving a domain to another [registrar](/en/glossary/registrar/) requires an authorization code — ICANN explains that [an Auth-Code is required for a domain holder to transfer a domain name from one registrar to another](https://www.icann.org/resources/pages/name-holder-faqs-2017-10-10-en#:~:text=An%20Auth%2DCode%20is%20required%20for%20a%20domain%20holder%20to%20transfer%20a%20domain%20name%20from%20one%20registrar%20to%20another) — and once that code is out and the transfer completes, the name is theirs. ICANN's current Transfer Policy still permits denial when a domain is within 60 days of its creation or a previous inter-registrar transfer, and it requires a 60-day inter-registrar lock after a Change of Registrant unless the registrar offered and the registrant chose an opt-out ([ICANN Transfer Policy, sections 3.7.5–3.8.5 and II.C.2](https://www.icann.org/en/contracted-parties/accredited-registrars/transfer-policy-01-06-2016-en#:~:text=3.7.5%20The%20transfer%20was%20requested%20within%2060%20days)). Those current 60-day rules were not comprehensively replaced by a universal 30-day lock, and none of them helps you if you've already given the name to a thief.

A close cousin of this scam targets the name you're *buying*, not selling: the seller takes payment and never releases the [auth code](/en/glossary/auth-code/), or releases one that doesn't work. The principle is identical from either side. Whoever moves first without a neutral party holding the other half of the deal is the one exposed.
...
**Always use independently verified, escrow-backed settlement.** Use a reputable escrow service or [marketplace](/en/glossary/marketplace/), then independently verify its legal identity, licensing where applicable, and official domain before signing in or funding the deal. Open or join the transaction through the provider's official site and your authenticated account, not a link supplied by either counterparty. A buyer's refusal to use any independently verified neutral service is a reason to stop; merely suggesting a legitimate provider is not proof of fraud. Our full walkthrough of how this should look is in [domain escrow explained](/en/blog/domain-escrow-explained/) and the seller's checklist in [how to sell a domain name you own](/en/blog/how-to-sell-a-domain-name-you-own/).

**Verify the other side before you trust the deal.** Run a [WHOIS](/en/glossary/whois/) lookup, check the company the buyer claims to be, confirm email domains match, and be suspicious of free webmail addresses pretending to be corporate procurement. A two-minute identity check defuses the fake-buyer and fake-broker scams almost entirely.
...
[Namefi](https://namefi.io) narrows that gap by tokenizing ownership of real ICANN domains, making control of the name auditable while preserving DNS continuity through a handover. Token transfer alone, however, is not atomic payment-versus-ownership settlement: ERC-721's standard transfer functions [specify moving NFT ownership from one address to another](https://eips.ethereum.org/EIPS/eip-721#:~:text=Transfers%20the%20ownership%20of%20an%20NFT%20from%20one%20address%20to%20another%20address), not a required purchase-payment leg. If both legs are onchain, a specific sale contract can bind payment and ownership delivery in one transaction and revert if either leg fails; EIP-140 specifies that `REVERT` [stops execution and rolls back all state changes made so far](https://eips.ethereum.org/EIPS/eip-140#:~:text=The%20REVERT%20instruction%20will%20stop%20execution%2C%20roll%20back%20all%20state%20changes). That contract-level binding — not tokenization by itself — is what creates atomic delivery-versus-payment and reduces the "who moves first" problem. Without it, the parties still need escrow or another settlement mechanism. We dig into that shift in [how tokenized marketplaces replace escrow](/en/blog/how-tokenized-marketplaces-replace-escrow/).
...
- Wikipedia — [Bogus escrow (definition and mechanics of the fake-escrow scam)](https://en.wikipedia.org/wiki/Bogus_escrow#:~:text=The%20bogus%20escrow%20scam%20is%20a%20straightforward%20confidence%20trick%20in%20which%20a%20scammer%20operates%20a%20bogus%20escrow%20service)
...
- ICANN — [Transfer Policy](https://www.icann.org/en/contracted-parties/accredited-registrars/transfer-policy-01-06-2016-en) · [Registrant transfer FAQ](https://www.icann.org/resources/pages/name-holder-faqs-2017-10-10-en)
- Ethereum Improvement Proposals — [ERC-721 token-transfer semantics](https://eips.ethereum.org/EIPS/eip-721) · [EIP-140 transaction rollback semantics](https://eips.ethereum.org/EIPS/eip-140)
- California DFPI — [10 Tips to Avoid Online Escrow Fraud](https://dfpi.ca.gov/regulated-industries/escrow-law/online-escrow-companies/10-tips-to-avoid-online-escrow-fraud/)
- Escrow.com — [Fraud Prevention](https://www.escrow.com/what-is-escrow/fraud-prevention)
