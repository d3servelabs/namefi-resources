...
function matchesTrialOffer(
  candidateDomain: string,
  offer: TrialOffer,
): boolean {
  if (offer.type === 'EXACT_DOMAIN') {
    return candidateDomain === offer.normalizedDomainName;
  }

  return (
    candidateDomain !== offer.normalizedDomainName &&
    candidateDomain.endsWith(`.${offer.normalizedDomainName}`)
  );
}
...
async function main(): Promise<void> {
  ...
  const normalizedDomainName = requireStringFlag(args, 'domain').toLowerCase();
  ...
  const matchedOffers = offers.filter((offer) =>
    matchesTrialOffer(normalizedDomainName, offer),
  );

  printJson({
    env,
    normalizedDomainName,
    eligible: matchedOffers.length > 0,
    matchedOffers,
    guidance:
      matchedOffers.length > 0
        ? matchedOffers.map((offer) =>
            offer.type === 'PARENT_DOMAIN'
              ? `Eligible because ${normalizedDomainName} is a subdomain of ${offer.normalizedDomainName}`
              : `Eligible because ${normalizedDomainName} exactly matches ${offer.normalizedDomainName}`,
          )
        : ['Not eligible for the current trial offer list.'],
  });
}
