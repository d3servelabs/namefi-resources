...
function buildSheetGuidedAnimationSheetPrompt(input: {
  ...
  return [
    ...
    'Canvas: 1536x1024, clean dark motion-design board, high contrast, crisp readable typography.',
    ...
    `Custom sheet direction from the strategist: ${input.sheetPrompt.trim()}.`,
    'Required sheet layout: 4-6 labeled keyframe panels with time ranges, a clear timing scale from 0.0s to 8.0s, concise text labels, easing notes, and one focused breakdown row showing how the real logo shapes/letterforms form, trace, morph, or settle.',
    'Use text, numbers, captions, and timing scales for explanation; keep annotations outside or below the logo artwork when possible.',
    'Do not render arrows, arrowheads, chevrons, pointer icons, callout lines, guide marks, path lines, vector strokes, or motion trails that could be mistaken for part of the animation.',
...
function buildSheetGuidedAnimationPrompt(input: {
  ...
  return [
    ...
    'Use [Image 2] only as a private motion reference for composition, transformation, and easing guidance, not as a source of pixels or overlays.',
    `Brand-specific motion direction: ${input.direction.trim()}.`,
    ...
    'The only durable visual source is [Image 1]; if an element is not present in [Image 1] or derived directly from the logo motion, omit it.',
    'Do not render, copy, preserve, or animate any non-logo [Image 2] annotation artifacts in the video: arrows, labels, captions, timing bars, timing scales, timecodes, panel borders, callout lines, guide marks, storyboard frames, or explanatory text are planning instructions only, never visual content.',
    'If [Image 2] contains any annotation artifacts despite the motion-reference instructions, ignore them completely and render a clean logo animation.',
    'No new text, no extra symbols, no mascot characters, no scene cuts, no destructive effects, and no morphing into a different mark.',
    'End on a sharp, fully legible hero frame with the logo cleanly resolved.',
  ].join(' ');
}
