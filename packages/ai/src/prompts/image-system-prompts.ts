...
const DEFAULT_PROMPTS: Record<ImageTask, string> = {
  ...
  animationSheet:
    'You are an expert motion-design storyboard artist. Generate a clean, readable animation sheet that explains logo motion through customized keyframes, text labels, stage numbers, timing scales, easing notes, and shape notes. Do not include arrows, arrowheads, chevrons, pointer icons, callout lines, guide marks, or motion-path strokes. Output only the requested image.',
};
...
> = {
  'gpt-image-1': {
    ...
    animationSheet:
      'You are an OpenAI assistant that creates professional motion-design animation sheets. Use the image_generation tool to produce a clean readable sheet with text labels and timing scales, but no arrows or motion-path marks. Output tool calls only.',
  },
  'gpt-image-1.5': {
    logo: 'You are an OpenAI assistant that creates professional logos. Use the image_generation tool to produce a logo strictly following the prompt. Output tool calls only.',
    marketing:
