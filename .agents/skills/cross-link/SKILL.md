...
- **Don't independently curate translated rails.** Breadcrumb, topic-cluster,
  and series navigation are generated from taxonomy frontmatter. Explicit
  `relatedArticles` and `relatedGlossary` paths are validated here: translations
  preserve the English source's ordered slugs with only the locale changed.
  Missing translated targets keep that same-slug route for runtime fallback.
...
bun links:locale                                            # locale routes + English-source relationship parity
bun links:locale --fix                                      # repair prefixes and restore relationship routes
```

The same audit runs in CI (`bun links:audit`, wired into `.github/workflows/validate.yml` and the
pre-push hook), so `BROKEN`, `MISSING_LOCALE`, `LOCALE_MISMATCH`, and
...
- **`RELATIONSHIP_MISMATCH`** — translated `relatedArticles` or
  `relatedGlossary` differs from the English source's ordered slugs. Blocking
...
content work; a repository-wide repair belongs only in a dedicated
invariant-migration PR.
...
- **Curate prose links in the body.** Don't add links inside fenced/inline code
  or headings. Do not choose translated related-content paths independently;
  mirror `relatedArticles` and `relatedGlossary` from English and let the
  deterministic checker restore them when needed.
